...
static gint          linux_input_get_n_events     (PicmanController *controller);
static const gchar * linux_input_get_event_name   (PicmanController *controller,
                                                   gint            event_id);
static const gchar * linux_input_get_event_blurb  (PicmanController *controller,
                                                   gint            event_id);
...
static const PicmanModuleInfo linux_input_info =
{
  PICMAN_MODULE_ABI_VERSION,
  N_("Linux input event controller"),
  "Sven Neumann <sven@picman.org>, Michael Natterer <mitch@picman.org>",
  "v0.2",
...
{
  picman_input_device_store_register_types (module);
...
controller_linux_input_class_init (ControllerLinuxInputClass *klass)
{
  ...
  g_object_class_install_property (object_class, PROP_DEVICE_STORE,
                                   g_param_spec_object ("device-values",
                                                        NULL, NULL,
                                                        PICMAN_TYPE_INPUT_DEVICE_STORE,
                                                        G_PARAM_READABLE));
#endif

  controller_class->name            = _("Linux Input");
  controller_class->help_id         = "picman-controller-linux-input";
  controller_class->stock_id        = PICMAN_STOCK_CONTROLLER_LINUX_INPUT;
...
{
  controller->store = picman_input_device_store_new ();
...
static gint
linux_input_get_n_events (PicmanController *controller)
{
  return G_N_ELEMENTS (key_events) + G_N_ELEMENTS (rel_events);
}

static const gchar *
linux_input_get_event_name (PicmanController *controller,
                            gint            event_id)
{
  if (event_id < 0)
    {
...
static const gchar *
linux_input_get_event_blurb (PicmanController *controller,
                             gint            event_id)
{
  if (event_id < 0)
    {
                        ...
                        const gchar          *device)
{
  ...
  if (controller->device && strlen (controller->device))
    {
      if (controller->store)
        filename = picman_input_device_store_get_device_file (controller->store,
                                                            controller->device);
      else
  ...
  if (filename)
    ...
    {
      GError *error = picman_input_device_store_get_error (controller->store);
                        ...
                        gpointer      data)
{
  ...
  if (n_bytes == sizeof (struct input_event))
    {
      PicmanController      *controller = PICMAN_CONTROLLER (data);
      PicmanControllerEvent  cevent     = { 0, };
      gint                 i;
        ...
        case EV_KEY:
          ...
          for (i = 0; i < G_N_ELEMENTS (key_events); i++)
            if (ev.code == key_events[i].code)
              {
                cevent.any.type     = PICMAN_CONTROLLER_EVENT_TRIGGER;
                cevent.any.source   = controller;
                cevent.any.event_id = i;

                picman_controller_event (controller, &cevent);
        ...
        case EV_REL:
          ...
          for (i = 0; i < G_N_ELEMENTS (rel_events); i++)
            if (ev.code == rel_events[i].code)
              {
                cevent.any.type     = PICMAN_CONTROLLER_EVENT_VALUE;
                cevent.any.source   = controller;
