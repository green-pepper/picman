...
aux_window_filter (GdkXEvent *xevent,
                   ...
                   gpointer   data)
{
  PicmanInputDeviceStore *store = (PicmanInputDeviceStore *) data;
...
static GdkWindow *
create_aux_window (PicmanInputDeviceStore *store)
{
  GdkWindowAttr wa;
  GdkWindow *retval;
...
enum_devices (const DIDEVICEINSTANCEW *di,
              void                    *user_data)
{
  PicmanInputDeviceStore *store = (PicmanInputDeviceStore *) user_data;

  picman_input_device_store_add (store, &di->guidInstance);
...
picman_input_device_store_init (PicmanInputDeviceStore *store)
{
  ...
  if (!GetModuleHandleEx (GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
                          (LPCTSTR) &picman_input_device_store_init,
                          &thismodule))
    return;

  if ((store->window = create_aux_window (store)) == NULL)
    {
      g_set_error_literal (&store->error, PICMAN_MODULE_ERROR, PICMAN_MODULE_FAILED,
  ...
  if (FAILED ((hresult = (*p_DirectInput8Create) (thismodule,
                                                  ...
                                                  NULL))))
    {
      g_set_error (&store->error, PICMAN_MODULE_ERROR, PICMAN_MODULE_FAILED,
                             ...
                             const GUID           *guid)
{
  ...
  picman_input_device_store_insert (store, guidstring, name, didevice8);
                                ...
                                const gchar          *guid)
{
  ...
  if (picman_input_device_store_lookup (store, guid, &iter))
    {
...
static void
picman_input_device_store_device_added (LibHalContext *ctx,
                                      const char    *guid)
{
  PicmanInputDeviceStore *store = libhal_ctx_get_user_data (ctx);

  if (picman_input_device_store_add (store, udi))
    {
...
static void
picman_input_device_store_device_removed (LibHalContext *ctx,
                                        const char    *udi)
{
  PicmanInputDeviceStore *store = libhal_ctx_get_user_data (ctx);

  if (picman_input_device_store_remove (store, udi))
    {
                                         ...
                                         const gchar          *udi)
{
  ...
  if (picman_input_device_store_lookup (store, udi, &iter))
    {
