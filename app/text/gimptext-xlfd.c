...
static gchar * picman_text_get_xlfd_field (const gchar *fontname,
                                         gint         field_num,
                                         gchar       *buffer);
...
/**
 * picman_text_font_name_from_xlfd:
 ...
 **/
gchar *
picman_text_font_name_from_xlfd (const gchar *xlfd)
{
  gchar *fields[4];
  ...
  /*  family  */
  fields[i] = picman_text_get_xlfd_field (xlfd, XLFD_FAMILY, buffers[i]);
  if (fields[i])
    i++;

  /*  weight  */
  fields[i] = picman_text_get_xlfd_field (xlfd, XLFD_WEIGHT, buffers[i]);
  if (fields[i] && strcmp (fields[i], "medium"))
    i++;

  /*  slant  */
  fields[i] = picman_text_get_xlfd_field (xlfd, XLFD_SLANT, buffers[i]);
  if (fields[i])
    {
      switch (*fields[i])
        {
  ...
  /*  stretch  */
  fields[i] = picman_text_get_xlfd_field (xlfd, XLFD_SET_WIDTH, buffers[i]);
...
/**
 * picman_text_font_size_from_xlfd:
 * @xlfd: X Logical Font Description
 ...
 **/
gboolean
picman_text_font_size_from_xlfd (const gchar *xlfd,
                               gdouble     *size,
                               PicmanUnit    *size_unit)
{
  ...
  field = picman_text_get_xlfd_field (xlfd, XLFD_PIXELS, buffer);
  if (field)
    {
      *size      = atoi (field);
      *size_unit = PICMAN_UNIT_PIXEL;
      return TRUE;
    }

  field = picman_text_get_xlfd_field (xlfd, XLFD_POINTS, buffer);
  if (field)
    {
      *size      = atoi (field) / 10.0;
      *size_unit = PICMAN_UNIT_POINT;
      return TRUE;
    }
...
/**
 * picman_text_set_font_from_xlfd:
 * @text: a #PicmanText object
 * @xlfd: X Logical Font Description
 *
 * Attempts to extract font name and font size from @xlfd and sets
 * them on the #PicmanText object.
 **/
void
picman_text_set_font_from_xlfd (PicmanText    *text,
                              const gchar *xlfd)
{
  gchar    *font;
  gdouble   size;
  ...
  font = picman_text_font_name_from_xlfd (xlfd);

#if PICMAN_TEXT_DEBUG
  g_printerr ("XLFD: %s  font: %s\n", xlfd, font ? font : "(null)");
#endif

  if (picman_text_font_size_from_xlfd (xlfd, &size, &size_unit))
    {
      g_object_set (text,
                    "font-size",          size,
 ...
 **/
static gchar *
picman_text_get_xlfd_field (const gchar *fontname,
                          gint         field_num,
                          gchar       *buffer)
{
  const gchar *t1, *t2;
