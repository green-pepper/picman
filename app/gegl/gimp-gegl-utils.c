...
const gchar *
picman_interpolation_to_gegl_filter (PicmanInterpolationType interpolation)
{
  switch (interpolation)
    {
    case PICMAN_INTERPOLATION_NONE:    return "nearest";
    case PICMAN_INTERPOLATION_LINEAR:  return "linear";
    case PICMAN_INTERPOLATION_CUBIC:   return "cubic";
    case PICMAN_INTERPOLATION_NOHALO:  return "nohalo";
    case PICMAN_INTERPOLATION_LOHALO:  return "lohalo";
    default:
      break;
    }
...
GType
picman_gegl_get_op_enum_type (const gchar *operation,
...
GeglColor *
picman_gegl_color_new (const PicmanRGB *rgb)
{
  GeglColor *color;
...
static void
picman_gegl_progress_notify (GObject          *object,
                           ...
                           PicmanProgress     *progress)
{
  ...
  text = g_object_get_data (object, "picman-progress-text");

  if (text)
    {
      if (value == 0.0)
        {
          picman_progress_start (progress, text, FALSE);
          return;
        }
      else if (value == 1.0)
        {
...
void
picman_gegl_progress_connect (GeglNode     *node,
                            PicmanProgress *progress,
                            const gchar  *text)
{
  ...
  g_signal_connect (operation, "notify::progress",
                    G_CALLBACK (picman_gegl_progress_notify),
                    progress);

  if (text)
    g_object_set_data_full (operation,
                            "picman-progress-text", g_strdup (text),
