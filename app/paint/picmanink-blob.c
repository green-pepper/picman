 ...
 *
 * picmanink-blob.c: routines for manipulating scan converted convex polygons.
...
static PicmanBlob * picman_blob_new            (gint      y,
                                            gint      height);
static void       picman_blob_fill           (PicmanBlob *b,
                                            EdgeType *present);
static void       picman_blob_make_convex    (PicmanBlob *b,
                                            EdgeType *present);

#if 0
static void       picman_blob_line_add_pixel (PicmanBlob *b,
                                            gint      x,
                                            gint      y);
static void       picman_blob_line           (PicmanBlob *b,
                                            gint      x0,
                                            gint      y0,
                                            gint      x1,
                                            gint      y1);
 ...
 */
PicmanBlob *
picman_blob_polygon (PicmanBlobPoint *points,
                   gint           n_points)
{
  PicmanBlob *result;
  EdgeType *present;
  ...
  result = picman_blob_new (ymin, ymax - ymin + 1);
  ...
  picman_blob_fill (result, present);
  g_free (present);
                  ...
                  gdouble yq)
{
  ...
  return picman_blob_polygon (points, 4);
}
 ...
 */
PicmanBlob *
picman_blob_diamond (gdouble xc,
                   gdouble yc,
                   gdouble xp,
                   gdouble yp,
                   gdouble xq,
 ...
 */
PicmanBlob *
picman_blob_ellipse (gdouble xc,
                   gdouble yc,
                   gdouble xp,
                   gdouble yp,
                   gdouble xq,
                   gdouble yq)
{
  ...
  result = picman_blob_new (miny, maxy - miny + 1);
  present = g_new0 (EdgeType, result->height);
...
void
picman_blob_bounds (PicmanBlob *b,
                  gint     *x,
                  gint     *y,
                  gint     *width,
                  gint     *height)
...
PicmanBlob *
picman_blob_convex_union (PicmanBlob *b1,
                        PicmanBlob *b2)
{
  PicmanBlob *result;
  gint      y;
  ...
  y = MIN (b1->y, b2->y);
  result = picman_blob_new (y, MAX (b1->y + b1->height, b2->y + b2->height)-y);
  ...
  picman_blob_make_convex (result, present);
...
PicmanBlob *
picman_blob_duplicate (PicmanBlob *b)
{
  g_return_val_if_fail (b != NULL, NULL);

  return g_memdup (b, sizeof (PicmanBlob) +  sizeof (PicmanBlobSpan) * (b->height - 1));
}

#if 0
void
picman_blob_dump (PicmanBlob *b)
{
  gint i,j;
...
static PicmanBlob *
picman_blob_new (gint y,
               gint height)
{
  PicmanBlob *result;

  result = g_malloc (sizeof (PicmanBlob) +  sizeof (PicmanBlobSpan) * (height - 1));
...
static void
picman_blob_make_convex (PicmanBlob *b,
                       EdgeType *present)
{
...
static void
picman_blob_line_add_pixel (PicmanBlob *b,
                          gint      x,
                          gint      y)
{
  ...
  if (dy < dx)
    {
      ...
      picman_blob_line_add_pixel (b, x, y);
