...
#include "picmanparamspecs-desc.h"


static inline const gchar *
picman_param_spec_get_blurb (GParamSpec *pspec)
{
...
static gchar *
picman_param_spec_boolean_desc (GParamSpec *pspec)
{
  const gchar *blurb = picman_param_spec_get_blurb (pspec);

  return g_strconcat (blurb, " (TRUE or FALSE)", NULL);
}

static gchar *
picman_param_spec_int32_desc (GParamSpec *pspec)
{
...
static gchar *
picman_param_spec_double_desc (GParamSpec *pspec)
{
...
static gchar *
picman_param_spec_enum_desc (GParamSpec *pspec)
{
  const gchar    *blurb      = picman_param_spec_get_blurb (pspec);
  ...
  if (PICMAN_IS_PARAM_SPEC_ENUM (pspec))
    excluded = PICMAN_PARAM_SPEC_ENUM (pspec)->excluded_values;
  else
    excluded = NULL;
  ...
  for (i = 0, n = 0, enum_value = enum_class->values;
       ...
       i++, enum_value++)
    {
      ...
      if (G_LIKELY (g_str_has_prefix (enum_value->value_name, "PICMAN_")))
        name = picman_canonicalize_identifier (enum_value->value_name + 5);
      else
        name = picman_canonicalize_identifier (enum_value->value_name);
 ...
 */
gchar *
picman_param_spec_get_desc (GParamSpec *pspec)
{
  g_return_val_if_fail (G_IS_PARAM_SPEC (pspec), NULL);

  if (PICMAN_IS_PARAM_SPEC_UNIT (pspec))
    {
    }
  else if (PICMAN_IS_PARAM_SPEC_INT32 (pspec))
    {
      return picman_param_spec_int32_desc (pspec);
    }
  else
    {
      switch (G_TYPE_FUNDAMENTAL (pspec->value_type))
        {
        case G_TYPE_BOOLEAN:
          return picman_param_spec_boolean_desc (pspec);

        case G_TYPE_DOUBLE:
          return picman_param_spec_double_desc (pspec);

        case G_TYPE_ENUM:
          return picman_param_spec_enum_desc (pspec);
        }
    }
