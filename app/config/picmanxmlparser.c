...
#include "picmanxmlparser.h"


struct _PicmanXmlParser
{
...
/**
 * picman_xml_parser_new:
 * @markup_parser: a #GMarkupParser
 * @user_data: user data to pass to #GMarkupParser functions
 *
 * PicmanXmlParser is a thin wrapper around GMarkupParser. This function
 * creates one for you and sets up a GMarkupParseContext.
 *
 * Return value: a new #PicmanXmlParser
 **/
PicmanXmlParser *
picman_xml_parser_new (const GMarkupParser *markup_parser,
                     gpointer             user_data)
{
  PicmanXmlParser *parser;

  g_return_val_if_fail (markup_parser != NULL, NULL);

  parser = g_slice_new (PicmanXmlParser);
 ...
 * @parser: a #PicmanXmlParser
 * @filename: name of a file to parse
 * @error: return location for possible errors
 *
 * This function creates a GIOChannel for @filename and calls
 * picman_xml_parser_parse_io_channel() for you.
 *
 * Return value: %TRUE on success, %FALSE otherwise
 **/
gboolean
picman_xml_parser_parse_file (PicmanXmlParser  *parser,
                            ...
                            GError        **error)
{
  ...
  success = picman_xml_parser_parse_io_channel (parser, io, error);
 ...
 **/
gboolean
picman_xml_parser_parse_fd (PicmanXmlParser  *parser,
                          gint            fd,
                          GError        **error)
{
  GIOChannel *io;
...
/**
 * picman_xml_parser_parse_io_channel:
 * @parser: a #PicmanXmlParser
 * @io: a #GIOChannel
 * @error: return location for possible errors
 *
 ...
 **/
gboolean
picman_xml_parser_parse_io_channel (PicmanXmlParser  *parser,
                                  GIOChannel     *io,
                                  GError        **error)
{
  GIOStatus    status;
  ...
  if (g_strcmp0 (io_encoding, "UTF-8"))
    {
      g_warning ("picman_xml_parser_parse_io_channel():\n"
...
/**
 * picman_xml_parser_parse_buffer:
 * @parser: a #PicmanXmlParser
 ...
 **/
gboolean
picman_xml_parser_parse_buffer (PicmanXmlParser  *parser,
                              const gchar    *buffer,
                              gssize          len,
                              GError        **error)
{
 ...
 **/
void
picman_xml_parser_free (PicmanXmlParser *parser)
{
  g_return_if_fail (parser != NULL);

  g_markup_parse_context_free (parser->context);
  g_slice_free (PicmanXmlParser, parser);
}
