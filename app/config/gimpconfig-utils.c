...
static void
picman_config_connect_notify (GObject    *src,
      ...
      if (dest_spec                                         &&
          ...
          (dest_spec->flags & G_PARAM_CONSTRUCT_ONLY) == 0)
        {
          ...
          g_signal_handlers_block_by_func (dest,
                                           picman_config_connect_notify, src);
 ...
 **/
void
picman_config_connect (GObject     *a,
                     GObject     *b,
                     const gchar *property_name)
{
  ...
  g_signal_connect_object (a, signal_name,
                           G_CALLBACK (picman_config_connect_notify),
                           b, 0);
...
static void
picman_config_connect_full_notify (GObject    *src,
                                 GParamSpec *param_spec,
                                 GObject    *dest)
{
  if (param_spec->flags & G_PARAM_READABLE)
    {
      ...
      if (dest_spec                                         &&
          ...
          (dest_spec->flags & G_PARAM_CONSTRUCT_ONLY) == 0)
        {
          ...
          g_signal_handlers_block_by_func (dest,
                                           picman_config_connect_full_notify, src);
 ...
 **/
void
picman_config_connect_full (GObject     *a,
                          GObject     *b,
                          ...
                          const gchar *property_name_b)
{
  ...
  g_signal_connect_object (a, signal_name,
                           G_CALLBACK (picman_config_connect_full_notify),
                           b, 0);
...
/**
 * picman_config_disconnect:
 * @a: a #GObject
 * @b: another #GObject
 *
 * Removes a connection between @dest and @src that was previously set
 * up using picman_config_connect().
 **/
void
picman_config_disconnect (GObject *a,
                        GObject *b)
{
  g_return_if_fail (G_IS_OBJECT (a) && G_IS_OBJECT (b));
