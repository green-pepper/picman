...
#define PICMAN_RC_UNKNOWN_TOKENS "picman-rc-unknown-tokens"
...
static void  picman_rc_destroy_unknown_tokens (GSList *unknown_tokens);


/**
 * picman_rc_add_unknown_token:
 * @config: a #GObject.
 ...
 * purpose is to attach additional data to a #PicmanConfig object that
 ...
 * default picman_config_serialize() implementation does not serialize
 * unknown tokens.
 *
 ...
 **/
void
picman_rc_add_unknown_token (PicmanConfig  *config,
                           const gchar *key,
                           const gchar *value)
{
  PicmanConfigToken *token;
          ...
          if (value)
          ...
          else
            {
              ...
              g_object_set_data_full (G_OBJECT (config),
                                      PICMAN_RC_UNKNOWN_TOKENS,
                                      unknown_tokens,
                     (GDestroyNotify) picman_rc_destroy_unknown_tokens);
            }
  ...
  token = g_slice_new (PicmanConfigToken);
...
/**
 * picman_rc_lookup_unknown_token:
 * @config: a #GObject.
 * @key: a nul-terminated string to identify the value.
 *
 * This function retrieves data that was previously attached using
 * picman_rc_add_unknown_token(). You should not free or modify
 * the returned string.
 *
 * Returns: a pointer to a constant string.
 **/
const gchar *
picman_rc_lookup_unknown_token (PicmanConfig  *config,
                              const gchar *key)
{
  GSList          *unknown_tokens;
  GSList          *list;
  ...
  for (list = unknown_tokens; list; list = g_slist_next (list))
    {
      PicmanConfigToken *token = list->data;
...
/**
 * picman_rc_foreach_unknown_token:
 * @config: a #GObject.
 ...
 **/
void
picman_rc_foreach_unknown_token (PicmanConfig            *config,
...
static void
picman_rc_destroy_unknown_tokens (GSList *unknown_tokens)
{
  GSList *list;
  ...
  for (list = unknown_tokens; list; list = g_slist_next (list))
    {
      ...
      g_free (token->key);
      g_free (token->value);
      g_slice_free (PicmanConfigToken, token);
    }
