...
void
picman_debug_enable_instances (void)
{
...
void
picman_debug_add_instance (GObject      *instance,
                         GObjectClass *klass)
{
  if (class_hash)
    {
...
void
picman_debug_remove_instance (GObject *instance)
{
  if (class_hash)
    {
      GHashTable  *instance_hash;
...
static void
picman_debug_instance_foreach (GObject *instance)
{
  g_printerr ("  \'%s\': ref_count = %d\n",
              PICMAN_IS_OBJECT (instance) ?
              picman_object_get_name (instance) : "GObject",
              instance->ref_count);
}

static void
picman_debug_class_foreach (const gchar *type_name,
                          GHashTable  *instance_hash)
{
  ...
  g_hash_table_foreach (instance_hash,
                        (GHFunc) picman_debug_instance_foreach,
                        NULL);
}

void
picman_debug_instances (void)
{
  if (class_hash)
    {
      g_hash_table_foreach (class_hash,
                            (GHFunc) picman_debug_class_foreach,
                            NULL);
    }
}
