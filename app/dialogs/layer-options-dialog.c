...
#include "core/picmandrawable-filter.h"
...
LayerOptionsDialog *
layer_options_dialog_new (PicmanImage    *image,
                          PicmanLayer    *layer,
                          ...
                          const gchar  *help_id)
{
  ...
  if (layer)
    viewable = PICMAN_VIEWABLE (layer);
  else
  ...
  if (! layer)
    {
      ...
      options->size_se = picman_size_entry_new (1, PICMAN_UNIT_PIXEL, "%a",
                                              TRUE, TRUE, FALSE, 10,
      ...
      picman_size_entry_add_field (PICMAN_SIZE_ENTRY (options->size_se),
      ...
      picman_size_entry_set_unit (PICMAN_SIZE_ENTRY (options->size_se),
                                PICMAN_UNIT_PIXEL);

      picman_size_entry_set_resolution (PICMAN_SIZE_ENTRY (options->size_se), 0,
                                      xres, FALSE);
      picman_size_entry_set_resolution (PICMAN_SIZE_ENTRY (options->size_se), 1,
                                      yres, FALSE);

      picman_size_entry_set_refval_boundaries (PICMAN_SIZE_ENTRY (options->size_se), 0,
                                             PICMAN_MIN_IMAGE_SIZE,
                                             PICMAN_MAX_IMAGE_SIZE);
      picman_size_entry_set_refval_boundaries (PICMAN_SIZE_ENTRY (options->size_se), 1,
                                             PICMAN_MIN_IMAGE_SIZE,
                                             PICMAN_MAX_IMAGE_SIZE);

      picman_size_entry_set_size (PICMAN_SIZE_ENTRY (options->size_se), 0,
                                0, picman_image_get_width  (image));
      picman_size_entry_set_size (PICMAN_SIZE_ENTRY (options->size_se), 1,
                                0, picman_image_get_height (image));

      picman_size_entry_set_refval (PICMAN_SIZE_ENTRY (options->size_se), 0,
                                  picman_image_get_width  (image));
      picman_size_entry_set_refval (PICMAN_SIZE_ENTRY (options->size_se), 1,
                                  picman_image_get_height (image));

      /*  The radio frame  */
      frame = picman_enum_radio_frame_new_with_range (PICMAN_TYPE_FILL_TYPE,
  ...
  else
    {
      ...
      if (picman_item_is_text_layer (PICMAN_ITEM (layer)))
        {
          ...
          gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (options->rename_toggle),
                                        PICMAN_TEXT_LAYER (layer)->auto_rename);
      ...
      frame = picman_frame_new ("Active Filters");
      gtk_box_pack_start (GTK_BOX (vbox), frame, TRUE, TRUE, 0);
      gtk_widget_show (frame);

      filters = picman_drawable_get_filters (PICMAN_DRAWABLE (layer));

      view = picman_container_tree_view_new (filters, context,
                                    ...
                                    LayerOptionsDialog *options)
{
  if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)) &&
      picman_item_is_text_layer (PICMAN_ITEM (options->layer)))
    {
      PicmanTextLayer *text_layer = PICMAN_TEXT_LAYER (options->layer);
      PicmanText      *text       = picman_text_layer_get_text (text_layer);

      if (text && text->text)
        {
          gchar *name = picman_utf8_strtrim (text->text, 30);
