...
static const PicmanActionEntry channels_actions[] =
{
  { "channels-popup", PICMAN_STOCK_CHANNELS,
  ...
  { "channels-raise-to-top", GTK_STOCK_GOTO_TOP,
    ...
    PICMAN_HELP_CHANNEL_RAISE_TO_TOP },
  ...
  { "channels-lower-to-bottom", GTK_STOCK_GOTO_BOTTOM,
    ...
    PICMAN_HELP_CHANNEL_LOWER_TO_BOTTOM }
};

static const PicmanEnumActionEntry channels_to_selection_actions[] =
{
  { "channels-selection-replace", PICMAN_STOCK_SELECTION_REPLACE,
    ...
    PICMAN_HELP_CHANNEL_SELECTION_REPLACE },

  { "channels-selection-add", PICMAN_STOCK_SELECTION_ADD,
    ...
    PICMAN_HELP_CHANNEL_SELECTION_ADD },

  { "channels-selection-subtract", PICMAN_STOCK_SELECTION_SUBTRACT,
    ...
    PICMAN_HELP_CHANNEL_SELECTION_SUBTRACT },

  { "channels-selection-intersect", PICMAN_STOCK_SELECTION_INTERSECT,
    ...
    PICMAN_HELP_CHANNEL_SELECTION_INTERSECT }
};


void
channels_actions_setup (PicmanActionGroup *group)
{
  picman_action_group_add_actions (group, "channels-action",
                                 channels_actions,
                                 G_N_ELEMENTS (channels_actions));

  picman_action_group_add_enum_actions (group, "channels-action",
...
void
channels_actions_update (PicmanActionGroup *group,
                         gpointer         data)
{
  ...
  if (image)
    {
      ...
      if (PICMAN_IS_COMPONENT_EDITOR (data))
        {
          if (PICMAN_COMPONENT_EDITOR (data)->clicked_component != -1)
            component = TRUE;
        }
      else
        {
          ...
          if (channel)
            {
              ...
              channel_list = picman_item_get_container_iter (PICMAN_ITEM (channel));
