...
static const PicmanActionEntry dock_actions[] =
{
  { "dock-move-to-screen-menu", PICMAN_STOCK_MOVE_TO_SCREEN,
    NC_("dock-action", "M_ove to Screen"), NULL, NULL, NULL,
    PICMAN_HELP_DOCK_CHANGE_SCREEN },

  { "dock-close", GTK_STOCK_CLOSE,
    NC_("dock-action", "Close Dock"), "", NULL,
    G_CALLBACK (window_close_cmd_callback),
    PICMAN_HELP_DOCK_CLOSE },
...
void
dock_actions_setup (PicmanActionGroup *group)
{
  picman_action_group_add_actions (group, "dock-action",
                                 dock_actions,
                                 G_N_ELEMENTS (dock_actions));

  picman_action_group_add_toggle_actions (group, "dock-action",
                                        dock_toggle_actions,
                                        G_N_ELEMENTS (dock_toggle_actions));

  window_actions_setup (group, PICMAN_HELP_DOCK_CHANGE_SCREEN);
}

void
dock_actions_update (PicmanActionGroup *group,
                     gpointer         data)
{
...
#define SET_VISIBLE(action,active) \
  ...
  if (PICMAN_IS_DOCK_WINDOW (toplevel))
    {
      PicmanDockWindow *dock_window = PICMAN_DOCK_WINDOW (toplevel);
      gboolean show_image_menu = ! picman_dock_window_has_toolbox (dock_window);
      ...
      if (show_image_menu)
        {
          ...
          SET_ACTIVE ("dock-show-image-menu",    picman_dock_window_get_show_image_menu (dock_window));
          SET_ACTIVE ("dock-auto-follow-active", picman_dock_window_get_auto_follow_active (dock_window));
        }
      else
        {
  ...
  else if (PICMAN_IS_IMAGE_WINDOW (toplevel))
    {
      SET_VISIBLE ("dock-show-image-menu",    FALSE);
      SET_VISIBLE ("dock-auto-follow-active", FALSE);
    }
