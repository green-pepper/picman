                        ...
                        gboolean     full_copy)
{
  ...
  args = picman_value_array_new (n_params);
  ...
  for (i = 0; i < n_params; i++)
    {
      ...
      type = picman_pdb_compat_arg_type_to_gtype (params[i].type);
      ...
      if (i > 0 || ! return_values)
        {
          ...
          if (pspec_index < n_pspecs)
            {
              PicmanPDBArgType pspec_arg_type;

              pspec_arg_type = picman_pdb_compat_arg_type_from_gtype
      ...
      switch (picman_pdb_compat_arg_type_from_gtype (type))
        {
        case PICMAN_PDB_INT32:
          if (G_VALUE_HOLDS_INT (&value))
            ...
            {
              g_printerr ("%s: unhandled PICMAN_PDB_INT32 type: %s\n",
        ...
        case PICMAN_PDB_INT32ARRAY:
          count = g_value_get_int (picman_value_array_index (args, i - 1));
          if (full_copy)
            picman_value_set_int32array (&value,
                                       params[i].data.d_int32array,
                                       count);
          else
            picman_value_set_static_int32array (&value,
                                              params[i].data.d_int32array,
                                              count);
          break;
        ...
        case PICMAN_PDB_INT16ARRAY:
          count = g_value_get_int (picman_value_array_index (args, i - 1));
          if (full_copy)
            picman_value_set_int16array (&value,
                                       params[i].data.d_int16array,
                                       count);
          else
            picman_value_set_static_int16array (&value,
                                              params[i].data.d_int16array,
                                              count);
          break;
        ...
        case PICMAN_PDB_INT8ARRAY:
          count = g_value_get_int (picman_value_array_index (args, i - 1));
          if (full_copy)
            picman_value_set_int8array (&value,
                                      params[i].data.d_int8array,
                                      count);
          else
            picman_value_set_static_int8array (&value,
                                             params[i].data.d_int8array,
                                             count);
          break;
        ...
        case PICMAN_PDB_FLOATARRAY:
          count = g_value_get_int (picman_value_array_index (args, i - 1));
          if (full_copy)
            picman_value_set_floatarray (&value,
                                       params[i].data.d_floatarray,
                                       count);
          else
            picman_value_set_static_floatarray (&value,
                                              params[i].data.d_floatarray,
                                              count);
          break;
        ...
        case PICMAN_PDB_STRINGARRAY:
          count = g_value_get_int (picman_value_array_index (args, i - 1));
          if (full_copy)
            picman_value_set_stringarray (&value,
                                        (const gchar **) params[i].data.d_stringarray,
                                        count);
          else
            picman_value_set_static_stringarray (&value,
                                               (const gchar **) params[i].data.d_stringarray,
                                               count);
          break;

        case PICMAN_PDB_COLOR:
          picman_value_set_rgb (&value, &params[i].data.d_color);
          break;
        ...
        case PICMAN_PDB_COLORARRAY:
          count = g_value_get_int (picman_value_array_index (args, i - 1));
          if (full_copy)
            picman_value_set_colorarray (&value,
                                      params[i].data.d_colorarray,
                                      count);
          else
            picman_value_set_static_colorarray (&value,
                                             params[i].data.d_colorarray,
                                             count);
          break;
...
GPParam *
plug_in_args_to_params (PicmanValueArray *args,
                        gboolean        full_copy)
{
  GPParam *params;
  gint     length;
  ...
  params = g_new0 (GPParam, picman_value_array_length (args));

  length = picman_value_array_length (args);

  for (i = 0; i < length; i++)
    {
      GValue *value = picman_value_array_index (args, i);

      params[i].type =
        picman_pdb_compat_arg_type_from_gtype (G_VALUE_TYPE (value));
      ...
      switch (params[i].type)
        ...
        case PICMAN_PDB_INT32ARRAY:
          if (full_copy)
            params[i].data.d_int32array = picman_value_dup_int32array (value);
          else
            params[i].data.d_int32array = (gint32 *) picman_value_get_int32array (value);
          break;

        case PICMAN_PDB_INT16ARRAY:
          if (full_copy)
            params[i].data.d_int16array = picman_value_dup_int16array (value);
          else
            params[i].data.d_int16array = (gint16 *) picman_value_get_int16array (value);
          break;

        case PICMAN_PDB_INT8ARRAY:
          if (full_copy)
            params[i].data.d_int8array = picman_value_dup_int8array (value);
          else
            params[i].data.d_int8array = (guint8 *) picman_value_get_int8array (value);
          break;

        case PICMAN_PDB_FLOATARRAY:
          if (full_copy)
            params[i].data.d_floatarray = picman_value_dup_floatarray (value);
          else
            params[i].data.d_floatarray = (gdouble *) picman_value_get_floatarray (value);
          break;

        case PICMAN_PDB_STRINGARRAY:
          if (full_copy)
            params[i].data.d_stringarray = picman_value_dup_stringarray (value);
          else
            params[i].data.d_stringarray = (gchar **) picman_value_get_stringarray (value);
          break;

        case PICMAN_PDB_COLOR:
          picman_value_get_rgb (value, &params[i].data.d_color);
          break;
        ...
        case PICMAN_PDB_COLORARRAY:
          if (full_copy)
            params[i].data.d_colorarray = picman_value_dup_colorarray (value);
          else
            params[i].data.d_colorarray = (PicmanRGB *) picman_value_get_colorarray (value);
          break;
        ...
        case PICMAN_PDB_PARASITE:
          {
            PicmanParasite *parasite = (full_copy ?
