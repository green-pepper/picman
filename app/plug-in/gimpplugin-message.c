...
static void picman_plug_in_handle_quit             (PicmanPlugIn      *plug_in);
static void picman_plug_in_handle_tile_request     (PicmanPlugIn      *plug_in,
                                                  GPTileReq       *request);
static void picman_plug_in_handle_tile_put         (PicmanPlugIn      *plug_in,
                                                  GPTileReq       *request);
static void picman_plug_in_handle_tile_get         (PicmanPlugIn      *plug_in,
                                                  GPTileReq       *request);
static void picman_plug_in_handle_proc_run         (PicmanPlugIn      *plug_in,
                                                  GPProcRun       *proc_run);
static void picman_plug_in_handle_proc_return      (PicmanPlugIn      *plug_in,
                                                  GPProcReturn    *proc_return);
static void picman_plug_in_handle_temp_proc_return (PicmanPlugIn      *plug_in,
                                                  GPProcReturn    *proc_return);
static void picman_plug_in_handle_proc_install     (PicmanPlugIn      *plug_in,
                                                  GPProcInstall   *proc_install);
static void picman_plug_in_handle_proc_uninstall   (PicmanPlugIn      *plug_in,
                                                  GPProcUninstall *proc_uninstall);
static void picman_plug_in_handle_extension_ack    (PicmanPlugIn      *plug_in);
static void picman_plug_in_handle_has_init         (PicmanPlugIn      *plug_in);


/*  public functions  */

void
picman_plug_in_handle_message (PicmanPlugIn      *plug_in,
                             PicmanWireMessage *msg)
{
  ...
  switch (msg->type)
    {
    case GP_QUIT:
      picman_plug_in_handle_quit (plug_in);
      break;
    ...
    case GP_TILE_REQ:
      picman_plug_in_handle_tile_request (plug_in, msg->data);
      break;
    ...
    case GP_PROC_RUN:
      picman_plug_in_handle_proc_run (plug_in, msg->data);
      break;

    case GP_PROC_RETURN:
      picman_plug_in_handle_proc_return (plug_in, msg->data);
      break;
    ...
    case GP_TEMP_PROC_RETURN:
      picman_plug_in_handle_temp_proc_return (plug_in, msg->data);
      break;

    case GP_PROC_INSTALL:
      picman_plug_in_handle_proc_install (plug_in, msg->data);
      break;

    case GP_PROC_UNINSTALL:
      picman_plug_in_handle_proc_uninstall (plug_in, msg->data);
      break;

    case GP_EXTENSION_ACK:
      picman_plug_in_handle_extension_ack (plug_in);
      break;

    case GP_HAS_INIT:
      picman_plug_in_handle_has_init (plug_in);
      break;
    }
}
...
static void
picman_plug_in_handle_quit (PicmanPlugIn *plug_in)
{
  picman_plug_in_close (plug_in, FALSE);
}

static void
picman_plug_in_handle_tile_request (PicmanPlugIn *plug_in,
                                  GPTileReq  *request)
{
  g_return_if_fail (request != NULL);

  if (request->drawable_ID == -1)
    picman_plug_in_handle_tile_put (plug_in, request);
  else
    picman_plug_in_handle_tile_get (plug_in, request);
}

static void
picman_plug_in_handle_tile_put (PicmanPlugIn *plug_in,
                              GPTileReq  *request)
{
  ...
  drawable = (PicmanDrawable *) picman_item_get_by_ID (plug_in->manager->picman,
                                                   tile_info->drawable_ID);

  if (! PICMAN_IS_DRAWABLE (drawable))
    {
      picman_message (plug_in->manager->picman, NULL, PICMAN_MESSAGE_ERROR,
                    "Plug-In \"%s\"\n(%s)\n\n"
  ...
  else if (picman_item_is_removed (PICMAN_ITEM (drawable)))
    {
      picman_message (plug_in->manager->picman, NULL, PICMAN_MESSAGE_ERROR,
                    "Plug-In \"%s\"\n(%s)\n\n"
  ...
  if (tile_info->shadow)
    {
       ...
       *  killing it.
       */
      buffer = picman_drawable_get_shadow_buffer (drawable);

      picman_plug_in_cleanup_add_shadow (plug_in, drawable);
    }
  else
    {
      ...
      else if (picman_viewable_get_children (PICMAN_VIEWABLE (drawable)))
        {
          picman_message (plug_in->manager->picman, NULL, PICMAN_MESSAGE_ERROR,
                        "Plug-In \"%s\"\n(%s)\n\n"
  ...
  if (! picman_gegl_buffer_get_tile_rect (buffer,
                                        PICMAN_PLUG_IN_TILE_WIDTH,
                                        PICMAN_PLUG_IN_TILE_HEIGHT,
                                        tile_info->tile_num,
                                        &tile_rect))
    {
      picman_message (plug_in->manager->picman, NULL, PICMAN_MESSAGE_ERROR,
  ...
  if (! picman_plug_in_precision_enabled (plug_in))
    {
  ...
  if (tile_data.use_shm)
    {
      gegl_buffer_set (buffer, &tile_rect, 0, format,
                       picman_plug_in_shm_get_addr (plug_in->manager->shm),
                       GEGL_AUTO_ROWSTRIDE);
    }
  else
    {
...
static void
picman_plug_in_handle_tile_get (PicmanPlugIn *plug_in,
                              GPTileReq  *request)
{
  GPTileData       tile_data;
  PicmanWireMessage  msg;
...
static void
picman_plug_in_handle_proc_error (PicmanPlugIn          *plug_in,
                                ...
                                const GError        *error)
{
  switch (proc_frame->error_handler)
    {
    case PICMAN_PDB_ERROR_HANDLER_INTERNAL:
      if (error->domain == PICMAN_PDB_ERROR)
        {
          picman_message (plug_in->manager->picman,
    ...
    case PICMAN_PDB_ERROR_HANDLER_PLUGIN:
...
static void
picman_plug_in_handle_proc_run (PicmanPlugIn *plug_in,
                              GPProcRun  *proc_run)
{
  PicmanPlugInProcFrame *proc_frame;
  gchar               *canonical;
  ...
  canonical = picman_canonicalize_identifier (proc_run->name);
  ...
  if (! procedure)
    {
      proc_name = picman_pdb_lookup_compat_proc_name (plug_in->manager->picman->pdb,
                                                    canonical);
      ...
      if (proc_name)
        {
          ...
          if (plug_in->manager->picman->pdb_compat_mode == PICMAN_PDB_COMPAT_WARN)
            {
              picman_message (plug_in->manager->picman, NULL, PICMAN_MESSAGE_WARNING,
                            "Plug-In \"%s\"\n(%s)\n"
  ...
  /*  Execute the procedure even if picman_pdb_lookup_procedure()
   *  returned NULL, picman_pdb_execute_procedure_by_name_args() will
   *  return appropriate error return_vals.
   */
  picman_plug_in_manager_plug_in_push (plug_in->manager, plug_in);
  return_vals = picman_pdb_execute_procedure_by_name_args (plug_in->manager->picman->pdb,
                                                         ...
                                                         proc_name,
                                                         args);
  picman_plug_in_manager_plug_in_pop (plug_in->manager);

  picman_value_array_unref (args);

  if (error)
    {
      picman_plug_in_handle_proc_error (plug_in, proc_frame,
                                      canonical, error);
      g_error_free (error);
    }
   ...
   *  closed the plug-in (e.g. if the procedure is picman-quit)
   */
  if (plug_in->open)
    {
      GPProcReturn proc_return;
       ...
       *  since proc_name may have been remapped by picman->procedural_compat_ht
       *  and canonical may be different too.
       */
      proc_return.name    = proc_run->name;
      proc_return.nparams = picman_value_array_length (return_vals);
...
static void
picman_plug_in_handle_proc_return (PicmanPlugIn   *plug_in,
                                 GPProcReturn *proc_return)
{
  ...
  if (proc_frame->main_loop)
  ...
  else
    {
       ...
       */
      picman_plug_in_procedure_handle_return_values (PICMAN_PLUG_IN_PROCEDURE (proc_frame->procedure),
                                                   plug_in->manager->picman,
                                                   proc_frame->progress,
                                                   proc_frame->return_vals);
    }
...
static void
picman_plug_in_handle_temp_proc_return (PicmanPlugIn   *plug_in,
                                      GPProcReturn *proc_return)
{
  g_return_if_fail (proc_return != NULL);
  ...
  if (plug_in->temp_proc_frames)
    {
      ...
      picman_plug_in_main_loop_quit (plug_in);
...
static void
picman_plug_in_handle_proc_install (PicmanPlugIn    *plug_in,
                                  GPProcInstall *proc_install)
{
  PicmanPlugInProcedure *proc       = NULL;
  PicmanProcedure       *procedure  = NULL;
  gchar               *canonical;
  ...
  canonical = picman_canonicalize_identifier (proc_install->name);

  /*  Sanity check for array arguments  */

  for (i = 1; i < proc_install->nparams; i++)
    {
      if ((proc_install->params[i].type == PICMAN_PDB_INT32ARRAY ||
           proc_install->params[i].type == PICMAN_PDB_INT8ARRAY  ||
           proc_install->params[i].type == PICMAN_PDB_FLOATARRAY ||
           proc_install->params[i].type == PICMAN_PDB_STRINGARRAY ||
           proc_install->params[i].type == PICMAN_PDB_COLORARRAY)
          &&
          proc_install->params[i - 1].type != PICMAN_PDB_INT32)
        {
          picman_message (plug_in->manager->picman, NULL, PICMAN_MESSAGE_ERROR,
                        "Plug-In \"%s\"\n(%s)\n\n"
...
#undef VALIDATE_OR_NULL
  ...
  switch (proc_install->type)
    ...
    case PICMAN_EXTENSION:
      procedure = picman_plug_in_procedure_new (proc_install->type,
                                              plug_in->prog);
      break;

    case PICMAN_TEMPORARY:
      procedure = picman_temporary_procedure_new (plug_in);
      break;
    }
  ...
  proc->installed_during_init = (plug_in->call_mode == PICMAN_PLUG_IN_CALL_INIT);

  picman_object_take_name (PICMAN_OBJECT (procedure), canonical);
  picman_procedure_set_strings (procedure,
                              proc_install->name,
                              proc_install->blurb,
                              proc_install->help,
                              proc_install->author,
  ...
  picman_plug_in_procedure_set_image_types (proc, proc_install->image_types);

  for (i = 0; i < proc_install->nparams; i++)
    {
      GParamSpec *pspec =
        picman_pdb_compat_param_spec (plug_in->manager->picman,
      ...
      if (proc_install->menu_path[0] == '<')
        {
          ...
          if (! picman_plug_in_procedure_add_menu_path (proc,
                                                      proc_install->menu_path,
                                                      &error))
            {
              picman_message_literal (plug_in->manager->picman,
				    NULL, PICMAN_MESSAGE_WARNING,
				    error->message);
              g_clear_error (&error);
            }
        }
  ...
  switch (proc_install->type)
    ...
    case PICMAN_PLUGIN:
    case PICMAN_EXTENSION:
      picman_plug_in_def_add_procedure (plug_in->plug_in_def, proc);
      break;

    case PICMAN_TEMPORARY:
      picman_plug_in_add_temp_proc (plug_in, PICMAN_TEMPORARY_PROCEDURE (proc));
      break;
    }
...
static void
picman_plug_in_handle_proc_uninstall (PicmanPlugIn      *plug_in,
                                    GPProcUninstall *proc_uninstall)
{
  ...
  canonical = picman_canonicalize_identifier (proc_uninstall->name);

  proc = picman_plug_in_procedure_find (plug_in->temp_procedures, canonical);

  if (proc)
    picman_plug_in_remove_temp_proc (plug_in, PICMAN_TEMPORARY_PROCEDURE (proc));

  g_free (canonical);
}

static void
picman_plug_in_handle_extension_ack (PicmanPlugIn *plug_in)
{
...
static void
picman_plug_in_handle_has_init (PicmanPlugIn *plug_in)
{
  if (plug_in->call_mode == PICMAN_PLUG_IN_CALL_QUERY)
    {
      picman_plug_in_def_set_has_init (plug_in->plug_in_def, TRUE);
    }
  else
    {
      picman_message (plug_in->manager->picman, NULL, PICMAN_MESSAGE_ERROR,
