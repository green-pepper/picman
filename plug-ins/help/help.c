...
#define PICMAN_HELP_EXT_PROC        "extension-picman-help"
#define PICMAN_HELP_TEMP_EXT_PROC   "extension-picman-help-temp"
...
static PicmanHelpProgress * load_help_progress_new (void);
...
query (void)
{
  ...
  picman_install_procedure (PICMAN_HELP_EXT_PROC,
                          "", /* FIXME */
                          "", /* FIXME */
     ...
     PicmanParam       **return_vals)
{
  ...
  if (nparams == 4)
    {
      if (! picman_help_init (param[0].data.d_int32,
  ...
  if (status == PICMAN_PDB_SUCCESS)
    {
      ...
      picman_uninstall_temp_proc (PICMAN_HELP_TEMP_EXT_PROC);
    }
...
temp_proc_install (void)
{
  static const PicmanParamDef args[] =
  {
    { PICMAN_PDB_STRING, "procedure",    "The procedure of the browser to use" },
  ...
  picman_install_temp_proc (PICMAN_HELP_TEMP_EXT_PROC,
                          "DON'T USE THIS ONE",
                          "(Temporary procedure)",
			  "Sven Neumann <sven@picman.org>, "
			  "Michael Natterer <mitch@picman.org>"
...
load_help_idle (gpointer data)
{
  ...
  domain = picman_help_lookup_domain (idle_help->help_domain);
  ...
  if (domain)
    {
      ...
      locales = picman_help_parse_locales (idle_help->help_locales);

      if (! g_str_has_prefix (domain->help_uri, "file:"))
        progress = load_help_progress_new ();

      uri = picman_help_domain_map (domain, locales, idle_help->help_id,
      ...
      if (uri)
        {
          ...
          return_vals = picman_run_procedure (idle_help->procedure,
                                            &n_return_vals,
                                            PICMAN_PDB_STRING, uri,
                                            PICMAN_PDB_END);
