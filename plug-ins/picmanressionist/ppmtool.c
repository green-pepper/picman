...
struct _BrushHeader
{
  ...
  unsigned int   magic_number;/*  PICMAN brush magic number  */
...
static void
load_picman_brush (const gchar *fn, ppm_t *p)
{
  FILE                *f;
  ...
  if (!f)
    {
      g_printerr ("load_picman_brush: Unable to open file \"%s\"!\n",
      ...
      if (strcmp (line, "P5"))
        {
          ...
          g_printerr ("ppm_load: File \"%s\" not PPM/PGM? (line=\"%s\")%c\n",
                      picman_filename_to_utf8 (fn), line, 7);
          ppm_new (p, 10,10);
          return;
    }
    pgm = 1;
...
ppm_save (ppm_t *p, const char *fn)
{
  ...
  if (!f)
    {
      /*
       * picman_filename_to_utf8 () and g_strerror () return temporary strings
