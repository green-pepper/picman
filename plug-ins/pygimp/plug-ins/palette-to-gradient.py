...
def make_gradient(palette, num_segments, num_colors):
    gradient = pdb.picman_gradient_new(palette)

    if (num_segments > 1):
        pdb.picman_gradient_segment_range_split_uniform(gradient, 0, -1,
                                                      num_segments)
    ...
    for color_number in range(0,num_segments):
        ...
        color_left = pdb.picman_palette_entry_get_color(palette,
                                                      color_number)
        color_right = pdb.picman_palette_entry_get_color(palette,
                                                       color_number_next)
        pdb.picman_gradient_segment_set_left_color(gradient,
                                                 color_number, color_left,
                                                 100.0)
        pdb.picman_gradient_segment_set_right_color(gradient,
                                                  color_number, color_right,
                                                  100.0)
    pdb.picman_context_set_gradient(gradient)
    return gradient
