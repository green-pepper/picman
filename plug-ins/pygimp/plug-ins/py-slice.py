...
def pyslice(image, drawable, save_path, html_filename,
    ...
    for i in range(0, len(horz) + 1):
        ...
        for j in range(0, len(vert) + 1):
            ...
            picman.progress_update(progress)
...
def slice(image, drawable, image_path, image_basename, image_extension,
    ...
    if not drawable:
        ...
        if image.base_type == INDEXED:
            #picman_layer_new_from_drawable doesn't work for indexed images.
            ...
            while len (temp_image.layers) > 1:
                if temp_image.layers[0] != temp_drawable:
                    pdb.picman_image_remove_layer (temp_image, temp_image.layers[0])
                else:
                    pdb.picman_image_remove_layer (temp_image, temp_image.layers[1])
        else:
            temp_image = pdb.picman_image_new (drawable.width, drawable.height,
                                         image.base_type)
            temp_drawable = pdb.picman_layer_new_from_drawable (drawable, temp_image)
    ...
    if image_extension == "gif" and image.base_type == RGB:
        pdb.picman_image_convert_indexed (temp_image,NO_DITHER, MAKE_PALETTE, 255,
                                        True, False, False)
    if image_extension == "jpg" and image.base_type == INDEXED:
        pdb.picman_image_convert_rgb (temp_image)

    pdb.picman_file_save(temp_image, temp_drawable, filename, filename)

    picman.delete(temp_image)
    return src
...
class TableWriter:
    ...
    def open(self):
        out = '''<!--HTML SNIPPET GENERATED BY PICMAN

WARNING!! This is NOT a fully valid HTML document, it is rather a piece of
HTML generated by PICMAN's py-slice plugin that should be embedded in an HTML
...
<!--
End of the part generated by PICMAN
-->
"""
            self.write (out)
