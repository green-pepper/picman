...
static PyObject *
tile_flush(PyPicmanTile *self, PyObject *args)
{
    if (!PyArg_ParseTuple(args, ":flush"))
	return NULL;

    picman_tile_flush(self->tile);
...
PyObject *
pypicman_tile_new(PicmanTile *t, PyPicmanDrawable *drw)
{
    PyPicmanTile *self;

    self = PyObject_NEW(PyPicmanTile, &PyPicmanTile_Type);
...
static void
tile_dealloc(PyPicmanTile *self)
{
    picman_tile_unref(self->tile, FALSE);
...
static PyObject *
tile_get_uint_field(PyPicmanTile *self, void *closure)
{
...
static PyObject *
tile_get_dirty(PyPicmanTile *self, void *closure)
{
    return PyBool_FromLong(self->tile->dirty);
}

static PyObject *
tile_get_shadow(PyPicmanTile *self, void *closure)
{
    return PyBool_FromLong(self->tile->shadow);
}

static PyObject *
tile_get_drawable(PyPicmanTile *self, void *closure)
{
    return pypicman_drawable_new(self->tile->drawable, 0);
}


#define OFF(x) GINT_TO_POINTER(offsetof(PicmanTile, x))
...
static PyObject *
tile_repr(PyPicmanTile *self)
{
    PyObject *s;
    gchar *name;

    name = picman_item_get_name(self->tile->drawable->drawable_id);

    if (self->tile->shadow)
	s = PyString_FromFormat("<picman.Tile for drawable '%s' (shadow)>", name);
    else
	s = PyString_FromFormat("<picman.Tile for drawable '%s'>", name);
...
{
    return ((PyPicmanTile*)self)->tile->ewidth * ((PyPicmanTile*)self)->tile->eheight;
}

static PyObject *
tile_subscript(PyPicmanTile *self, PyObject *sub)
{
    PicmanTile *tile = self->tile;
    int bpp = tile->bpp;
    long x, y;
...
static int
tile_ass_sub(PyPicmanTile *self, PyObject *v, PyObject *w)
{
    PicmanTile *tile = self->tile;
    int bpp = tile->bpp, i;
    long x, y;
...
PyTypeObject PyPicmanTile_Type = {
    PyObject_HEAD_INIT(NULL)
    0,                                  /* ob_size */
    "picman.Tile",                        /* tp_name */
    sizeof(PyPicmanTile),                 /* tp_basicsize */
    0,                                  /* tp_itemsize */
    /* methods */
...
static PyObject *
pr_resize(PyPicmanPixelRgn *self, PyObject *args)
{
    int x, y, w, h;
    ...
    picman_pixel_rgn_resize(&(self->pr), x, y, w, h);
...
PyObject *
pypicman_pixel_rgn_new(PyPicmanDrawable *drawable, int x, int y,
		     int width, int height, int dirty, int shadow)
{
    PyPicmanPixelRgn *self;
    int drw_width;
    int drw_height;

    self = PyObject_NEW(PyPicmanPixelRgn, &PyPicmanPixelRgn_Type);

    if (self == NULL || drawable == NULL)
        return NULL;

    drw_width = picman_drawable_width(drawable->ID);
    drw_height = picman_drawable_height(drawable->ID);
    ...
    picman_pixel_rgn_init(&(self->pr), drawable->drawable, x, y, width, height,
			dirty, shadow);
...
static void
pr_dealloc(PyPicmanPixelRgn *self)
{
    Py_DECREF(self->drawable);
    PyObject_DEL(self);
}
...
{
    PyErr_SetString(pypicman_error, "Can't get size of pixel region");
    return -1;
}

static PyObject *
pr_subscript(PyPicmanPixelRgn *self, PyObject *key)
{
    PicmanPixelRgn *pr = &(self->pr);
    PyObject *x, *y;
    ...
    if (PyInt_Check(x)) {
        ...
        if (PyInt_Check(y)) {
            ...
            picman_pixel_rgn_get_pixel(pr, (guchar*)PyString_AS_STRING(ret),
                                     x1, y1);
            ...
            picman_pixel_rgn_get_col(pr, (guchar*)PyString_AS_STRING(ret),
                                   x1, y1, y2-y1);
	    }
        else {
        ...
        if (PyInt_Check(y)) {
            ...
            picman_pixel_rgn_get_row(pr, (guchar*)PyString_AS_STRING(ret),
                                   x1, y1, x2 - x1);
            ...
            ret = PyString_FromStringAndSize(NULL,
                                             pr->bpp * (x2 - x1) * (y2 - y1));
            picman_pixel_rgn_get_rect(pr, (guchar*)PyString_AS_STRING(ret),
                                    x1, y1, x2 - x1, y2 - y1);
	    }
        else {
            PyErr_SetString(PyExc_TypeError, "invalid y subscript");
...
static int
pr_ass_sub(PyPicmanPixelRgn *self, PyObject *v, PyObject *w)
{
    PicmanPixelRgn *pr = &(self->pr);
    PyObject *x, *y;
    const guchar *buf;
    ...
    if (PyInt_Check(x)) {
        ...
        if (PyInt_Check(y)) {
            ...
            picman_pixel_rgn_set_pixel(pr, buf, x1, y1);
            ...
            picman_pixel_rgn_set_col(pr, buf, x1, y1, y2 - y1);
        ...
        if (PyInt_Check(y)) {
            ...
            picman_pixel_rgn_set_row(pr, buf, x1, y1, x2 - x1);
            ...
            picman_pixel_rgn_set_rect(pr, buf, x1, y1, x2 - x1, y2 - y1);
...
static PyObject *
pr_get_drawable(PyPicmanPixelRgn *self, void *closure)
{
    return pypicman_drawable_new(self->pr.drawable, 0);
}

static PyObject *
pr_get_uint_field(PyPicmanPixelRgn *self, void *closure)
{
    gint offset = GPOINTER_TO_INT(closure);
    guint value;
    gchar *addr;
...
static PyObject *
pr_get_dirty(PyPicmanPixelRgn *self, void *closure)
{
    return PyBool_FromLong(self->pr.dirty);
}

static PyObject *
pr_get_shadow(PyPicmanPixelRgn *self, void *closure)
{
    return PyBool_FromLong(self->pr.shadow);
}

#define OFF(x) GINT_TO_POINTER(offsetof(PicmanPixelRgn, x))
...
static PyObject *
pr_repr(PyPicmanPixelRgn *self)
{
    PyObject *s;
    gchar *name;

    name = picman_item_get_name(self->drawable->drawable->drawable_id);
    s = PyString_FromFormat("<picman.PixelRgn for drawable '%s'>", name);
    g_free(name);

    return s;
}

PyTypeObject PyPicmanPixelRgn_Type = {
    PyObject_HEAD_INIT(NULL)
    0,                                  /* ob_size */
    "picman.PixelRgn",                    /* tp_name */
    sizeof(PyPicmanPixelRgn),             /* tp_basicsize */
    0,                                  /* tp_itemsize */
    /* methods */
...
static PyObject *
pf_get_pixel(PyPicmanPixelFetcher *self, PyObject *args, PyObject *kwargs)
{
    int x, y;
    guchar pixel[4];
    ...
    picman_pixel_fetcher_get_pixel(self->pf, x, y, pixel);

    return PyString_FromStringAndSize((char *)pixel, self->bpp);
}

static PyObject *
pf_put_pixel(PyPicmanPixelFetcher *self, PyObject *args, PyObject *kwargs)
{
    int x, y, len;
    guchar *pixel;
    ...
    picman_pixel_fetcher_put_pixel(self->pf, x, y, pixel);
...
static int
pf_length(PyPicmanPixelFetcher *self)
{
    PyErr_SetString(pypicman_error, "Can't get size of pixel fetcher");
    return -1;
}

static PyObject *
pf_subscript(PyPicmanPixelFetcher *self, PyObject *key)
{
    PyObject *py_x, *py_y;
    int x, y;
    guchar pixel[4];
...
static int
pf_ass_sub(PyPicmanPixelFetcher *self, PyObject *v, PyObject *w)
{
    PyObject *py_x, *py_y;
    int x, y, len;
    guchar *pixel;
...
static PyObject *
pf_get_bg_color(PyPicmanPixelFetcher *self, void *closure)
{
    return pypicman_rgb_new(&self->bg_color);
}

static int
pf_set_bg_color(PyPicmanPixelFetcher *self, PyObject *value, void *closure)
{
    if (value == NULL) {
    ...
    if (!pypicman_rgb_from_pyobject(value, &self->bg_color))
        return -1;

    picman_pixel_fetcher_set_bg_color(self->pf, &self->bg_color);

    return 0;
}

static PyObject *
pf_get_edge_mode(PyPicmanPixelFetcher *self, void *closure)
{
    return PyInt_FromLong(self->edge_mode);
}

static int
pf_set_edge_mode(PyPicmanPixelFetcher *self, PyObject *value, void *closure)
{
    if (value == NULL) {
    ...
    picman_pixel_fetcher_set_edge_mode(self->pf, self->edge_mode);
...
static void
pf_dealloc(PyPicmanPixelFetcher *self)
{
    picman_pixel_fetcher_destroy(self->pf);
...
static PyObject *
pf_repr(PyPicmanPixelFetcher *self)
{
    PyObject *s;
    char *name;
    ...
    if (self->shadow)
        s = PyString_FromFormat("<picman.PixelFetcher for drawable '%s' (shadow)>", name);
    else
        s = PyString_FromFormat("<picman.PixelFetcher for drawable '%s'>", name);
...
static int
pf_init(PyPicmanPixelFetcher *self, PyObject *args, PyObject *kwargs)
{
    PyPicmanDrawable *drw;
    gboolean shadow = FALSE;
    PicmanRGB bg_color = { 0.0, 0.0, 0.0, 1.0 };
    PicmanPixelFetcherEdgeMode edge_mode = PICMAN_PIXEL_FETCHER_EDGE_NONE;
    static char *kwlist[] = { "drawable", "shadow", "bg_color", "edge_mode",
	                      NULL };

    if (!PyArg_ParseTupleAndKeywords(args, kwargs,
                                     "O!|iO&i:picman.PixelFetcher.__init__",
                                     kwlist,
                                     &PyPicmanDrawable_Type, &drw, &shadow,
                                     pypicman_rgb_from_pyobject, &bg_color,
                                     &edge_mode))
        return -1;

    if(!drw->drawable)
        drw->drawable = picman_drawable_get(drw->ID);

    self->pf = picman_pixel_fetcher_new(drw->drawable, shadow);
    ...
    self->bpp = picman_drawable_bpp(drw->drawable->drawable_id);

    picman_pixel_fetcher_set_bg_color(self->pf, &bg_color);
    picman_pixel_fetcher_set_edge_mode(self->pf, edge_mode);

    return 0;
}

PyTypeObject PyPicmanPixelFetcher_Type = {
    PyObject_HEAD_INIT(NULL)
    0,                                  /* ob_size */
    "picman.PixelFetcher",                /* tp_name */
    sizeof(PyPicmanPixelFetcher),         /* tp_basicsize */
    0,                                  /* tp_itemsize */
    /* methods */
