                 ...
                 gint ph)
{
  ...
  picman_rgba_set (&lightcheck,
                 PICMAN_CHECK_LIGHT, PICMAN_CHECK_LIGHT, PICMAN_CHECK_LIGHT, 1.0);
  picman_rgba_set (&darkcheck,
                 PICMAN_CHECK_DARK, PICMAN_CHECK_DARK, PICMAN_CHECK_DARK, 1.0);
  picman_vector3_set (&p2, -1.0, -1.0, 0.0);
              ...
              if (f1)
                {
                  if (color.a == 0.0)
                    ...
                    picman_rgb_composite (&color, &lightcheck,
              ...
              else
                {
                  if (color.a == 0.0)
                    ...
                    picman_rgb_composite (&color, &darkcheck,
                                        PICMAN_RGB_COMPOSITE_BEHIND);
                }
            }
             ...
             gint ph)
{
  ...
  picman_vector_3d_to_2d (startx, starty, pw, ph,
              ...
              gint ypos)
{
  ...
  picman_vector_2d_to_3d (startx, starty, pw, ph, xpos, ypos,
...
draw_wireframe_plane (cairo_t *cr,
                      ...
                      gint ph)
{
  PicmanVector3 v1, v2, a, b, c, d, dir1, dir2;
  gint        cnt;
  ...
  picman_vector3_set (&v1, 0.5, 0.0, 0.0);
  picman_vector3_set (&v2, 0.0, 0.5, 0.0);

  picman_vector3_rotate (&v1,
                       picman_deg_to_rad (mapvals.alpha),
                       picman_deg_to_rad (mapvals.beta),
                       picman_deg_to_rad (mapvals.gamma));

  picman_vector3_rotate (&v2,
                       picman_deg_to_rad (mapvals.alpha),
                       picman_deg_to_rad (mapvals.beta),
                       picman_deg_to_rad (mapvals.gamma));

  dir1 = v1; picman_vector3_normalize (&dir1);
  dir2 = v2; picman_vector3_normalize (&dir2);

  fac = 1.0 / (gdouble) WIRESIZE;

  picman_vector3_mul (&dir1, fac);
  picman_vector3_mul (&dir2, fac);

  picman_vector3_add (&a, &mapvals.position, &v1);
  picman_vector3_sub (&b, &a, &v2);
  picman_vector3_add (&a, &a, &v2);
  picman_vector3_sub (&d, &mapvals.position, &v1);
  picman_vector3_sub (&d, &d, &v2);
  ...
  for (cnt = 0; cnt <= WIRESIZE; cnt++)
    {
      ...
      picman_vector3_sub (&a, &a, &dir1);
      picman_vector3_sub (&b, &b, &dir1);
      picman_vector3_add (&c, &c, &dir2);
      picman_vector3_add (&d, &d, &dir2);
    }
                       ...
                       gint ph)
{
  ...
  for (cnt = 0; cnt < WIRESIZE; cnt++)
    {
      ...
      picman_vector3_rotate (&p[cnt],
                           picman_deg_to_rad (mapvals.alpha),
                           picman_deg_to_rad (mapvals.beta),
                           picman_deg_to_rad (mapvals.gamma));
      picman_vector3_add (&p[cnt], &p[cnt], &mapvals.position);
    }
  ...
  picman_vector3_set (&p[cnt], 0.0, -0.35, 0.0);
  picman_vector3_rotate (&p[cnt],
                       picman_deg_to_rad (mapvals.alpha),
                       picman_deg_to_rad (mapvals.beta),
                       picman_deg_to_rad (mapvals.gamma));
  ...
  picman_vector3_set (&p[cnt+2], 0.0, 0.0, -0.35);
  picman_vector3_rotate (&p[cnt+2],
                       picman_deg_to_rad (mapvals.alpha),
                       picman_deg_to_rad (mapvals.beta),
                       picman_deg_to_rad (mapvals.gamma));
  ...
  p[cnt + 4] = p[cnt];
  picman_vector3_mul (&p[cnt + 4], -1.0);
  p[cnt + 5] = p[cnt + 1];

  picman_vector3_add (&p[cnt],     &p[cnt],     &mapvals.position);
  picman_vector3_add (&p[cnt + 2], &p[cnt + 2], &mapvals.position);
  picman_vector3_add (&p[cnt + 4], &p[cnt + 4], &mapvals.position);
                    ...
                    gint ph)
{
  ...
  scale = mapvals.scale;
  picman_vector3_mul (&scale, 0.5);

  picman_vector3_set (&p[0], -scale.x, -scale.y, scale.z);
  picman_vector3_set (&p[1],  scale.x, -scale.y, scale.z);
  picman_vector3_set (&p[2],  scale.x,  scale.y, scale.z);
  picman_vector3_set (&p[3], -scale.x,  scale.y, scale.z);

  picman_vector3_set (&p[4], -scale.x, -scale.y, -scale.z);
  picman_vector3_set (&p[5],  scale.x, -scale.y, -scale.z);
  picman_vector3_set (&p[6],  scale.x,  scale.y, -scale.z);
  picman_vector3_set (&p[7], -scale.x,  scale.y, -scale.z);
  ...
  for (i = 0; i < 8; i++)
    {
      ...
      picman_vector3_add (&p[i], &tmp, &mapvals.position);
    }
...
draw_wireframe_cylinder (cairo_t *cr,
                         ...
                         gint ph)
{
  PicmanVector3 p[2*8], a, axis, scale;
  gint        i;
  gdouble     cx1, cy1, cx2, cy2;
  gfloat      m[16], l, angle;
  ...
  picman_vector3_set (&axis, 0.0, 1.0, 0.0);

  for (i = 0; i < 8; i++)
    {
      rotatemat (angle, &axis, m);

      picman_vector3_set (&a, mapvals.cylinder_radius, 0.0, 0.0);
  ...
  for (i = 0; i < 16; i++)
    {
      ...
      picman_vector3_add (&p[i], &a, &mapvals.position);
    }
