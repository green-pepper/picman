/* Ripple --- image filter plug-in for PICMAN
...
#define PLUG_IN_ROLE    "picman-ripple"
...
static void      ripple             (PicmanDrawable     *drawable,
                                     PicmanPreview      *preview);

static gboolean  ripple_dialog      (PicmanDrawable     *drawable);
...
static RippleValues rvals =
{
  20,                          /* period      */
  5,                           /* amplitude   */
  PICMAN_ORIENTATION_HORIZONTAL, /* orientation */
  WRAP,                        /* edges       */
  SINE,                        /* waveform    */
  TRUE,                        /* antialias   */
  FALSE,                       /* tile        */
...
query (void)
{
  static const PicmanParamDef args[] =
  {
    ...
    { PICMAN_PDB_INT32,    "period",      "Period: number of pixels for one wave to complete" },
    { PICMAN_PDB_INT32,    "amplitude",   "Amplitude: maximum displacement of wave" },
    { PICMAN_PDB_INT32,    "orientation", "Orientation { ORIENTATION-HORIZONTAL (0), ORIENTATION-VERTICAL (1) }" },
    { PICMAN_PDB_INT32,    "edges",       "Edges { SMEAR (0), WRAP (1), BLANK (2) }" },
    { PICMAN_PDB_INT32,    "waveform",    "Waveform { SAWTOOTH (0), SINE (1) }" },
    { PICMAN_PDB_INT32,    "antialias",   "Antialias { TRUE, FALSE }" },
    { PICMAN_PDB_INT32,    "tile",        "Tileable { TRUE, FALSE }" }
  };
    ...
    case PICMAN_RUN_NONINTERACTIVE:
      ...
      if (nparams != 10)
      ...
      else
        {
          ...
          rvals.orientation = (param[5].data.d_int32) ? PICMAN_ORIENTATION_VERTICAL : PICMAN_ORIENTATION_HORIZONTAL;
          ...
          if (rvals.period < 1)
            {
              picman_message ("Ripple: period must be at least 1.\n");
              status = PICMAN_PDB_CALLING_ERROR;
            }
  ...
  if (status == PICMAN_PDB_SUCCESS)
    {
      ...
      if (picman_drawable_is_rgb (drawable->drawable_id) ||
        ...
        {
          picman_progress_init (_("Rippling"));
          ...
          if (run_mode == PICMAN_RUN_INTERACTIVE)
            picman_set_data (PLUG_IN_PROC, &rvals, sizeof (RippleValues));
        }
      else
        {
          /* picman_message ("ripple: cannot operate on indexed color images"); */
                 ...
                 gpointer  data)
{
  ...
  PicmanPixelFetcher *pft    = param->pft;
  ...
  if (rvals.antialias)
    {
      if (yi >= 0 && yi < height)
        picman_pixel_fetcher_get_pixel (pft, x, yi  , pixel[0]);
      else
        memset (pixel[0], 0, 4);

      if (yi_a >= 0 && yi_a < height)
        picman_pixel_fetcher_get_pixel (pft, x, yi_a, pixel[1]);
      else
        memset (pixel[1], 0, 4);
    ...
    {
      if (yi >= 0 && yi < height)
        picman_pixel_fetcher_get_pixel (pft, x, yi, dest);
      else
        memset (dest, 0, bpp);
    }
}
                   ...
                   gpointer  data)
{
  ...
  if (rvals.antialias)
    {
      if (xi >= 0 && xi < width)
        picman_pixel_fetcher_get_pixel (pft, xi,   y, pixel[0]);
      else
        memset (pixel[0], 0, 4);

      if (xi_a >= 0 && xi_a < width)
        picman_pixel_fetcher_get_pixel (pft, xi_a, y, pixel[1]);
      else
        memset (pixel[1], 0, 4);
    ...
    {
      if (xi >= 0 && xi < width)
        picman_pixel_fetcher_get_pixel (pft, xi, y, dest);
      else
        memset (dest, 0, bpp);
    }
}
...
static void
ripple (PicmanDrawable *drawable,
        PicmanPreview  *preview)
{
  RippleParam_t param;
  gint          edges;
  ...
  if (rvals.tile)
    {
      ...
      rvals.period = (param.width / (param.width / rvals.period) *
                      (rvals.orientation == PICMAN_ORIENTATION_HORIZONTAL) +
                      param.height / (param.height / rvals.period) *
                      (rvals.orientation == PICMAN_ORIENTATION_VERTICAL));
    }

  if (preview)
    {
      guchar *buffer, *d;
      gint    bpp = picman_drawable_bpp (drawable->drawable_id);
      gint    width, height;
      gint    x, y;
      gint    x1, y1;
      ...
      for (y = 0; y < height ; y++)
        for (x = 0; x < width ; x++)
          {
            if (rvals.orientation == PICMAN_ORIENTATION_VERTICAL)
...
static gboolean
ripple_dialog (PicmanDrawable *drawable)
{
  GtkWidget *dialog;
  GtkWidget *main_vbox;
  GtkWidget *preview;
  ...
  dialog = picman_dialog_new (_("Ripple"), PLUG_IN_ROLE,
                            NULL, 0,
  ...
  frame = picman_int_radio_group_new (TRUE, _("Orientation"),
  ...
  /*  Edges toggle box  */
  frame = picman_int_radio_group_new (TRUE, _("Edges"),
                                    G_CALLBACK (picman_radio_button_update),
  ...
  /*  Wave toggle box  */
  frame = picman_int_radio_group_new (TRUE, _("Wave Type"),
                                    G_CALLBACK (picman_radio_button_update),
