...
(define (script-fu-selection-rounded-rectangle image drawable radius concave)
  ...
  (if (= (car (picman-selection-is-empty image)) TRUE) (picman-selection-all image))
  (let* (
        (radius (/ radius 100)) ; convert from percentages
        (radius (min radius 1.0))
        (radius (max radius 0.0))
        (select-bounds (picman-selection-bounds image))
    ...
    (picman-image-select-rectangle image CHANNEL-OP-ADD
    ...
    ; top right
    (picman-image-select-ellipse image CHANNEL-OP-SUBTRACT
    ...
    (if (= concave FALSE)
      (begin
        (picman-image-select-ellipse image
                                   CHANNEL-OP-ADD
                                   select-x1
                                   select-y1
                                   ellipse-radius
