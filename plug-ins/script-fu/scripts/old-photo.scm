...
(define (script-fu-old-photo inImage inLayer inDefocus inBorderSize inSepia inMottle inCopy)
  ...
  (picman-image-undo-group-start inImage)
  ...
  (set! theLayer (car (picman-image-flatten theImage)))
  (if (= inDefocus TRUE)
  ...
  (if (= inSepia TRUE)
      (begin (picman-desaturate theLayer)
             (picman-brightness-contrast theLayer -20 -40)
             (picman-color-balance theLayer 0 TRUE 30 0 -30)
      )
  )
  (set! theWidth (car (picman-image-width theImage)))
  (set! theHeight (car (picman-image-height theImage)))
  (if (= inMottle TRUE)
      (let (
	    (mLayer (car (picman-layer-new theImage theWidth theHeight
					 RGBA-IMAGE "Mottle"
					 100 DARKEN-ONLY-MODE)))
	    )

             (picman-image-insert-layer theImage mLayer 0 0)
             (picman-selection-all theImage)
             (picman-edit-clear mLayer)
  ...
  (if (= inCopy TRUE)
      (begin  (picman-image-clean-all theImage)
              (picman-display-new theImage)
      )
  )
  (picman-selection-none inImage)
  (picman-image-undo-group-end inImage)
  (picman-displays-flush theImage)
  )
)
