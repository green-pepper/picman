...
#define __PICMAN_ENV_C__
#include "picmanenv.h"
#include "picmanversion.h"
 ...
 * SECTION: picmanenv
 * @title: picmanenv
 * @short_description: Functions to access the PICMAN environment.
 *
 * A set of functions to find the locations of PICMAN's data directories
 * and configuration files.
 **/


static gchar * picman_env_get_dir   (const gchar *picman_env_name,
...
const guint picman_major_version = PICMAN_MAJOR_VERSION;
const guint picman_minor_version = PICMAN_MINOR_VERSION;
const guint picman_micro_version = PICMAN_MICRO_VERSION;
 ...
 */
void
picman_env_init (gboolean plug_in)
{
  static gboolean picman_env_initialized = FALSE;

  if (picman_env_initialized)
    g_error ("picman_env_init() must only be called once!");

  picman_env_initialized = TRUE;

#ifndef G_OS_WIN32
  if (plug_in)
    {
      _picman_reloc_init_lib (NULL);
    }
  else if (_picman_reloc_init (NULL))
    {
      /* Set $LD_LIBRARY_PATH to ensure that plugins can be loaded. */

      const gchar *ldpath = g_getenv ("LD_LIBRARY_PATH");
      gchar       *libdir = g_build_filename (picman_installation_directory (),
                                              "lib",
                                              NULL);
 ...
 * picman_directory:
 *
 * Returns the user-specific PICMAN settings directory. If the
 * environment variable PICMAN2_DIRECTORY exists, it is used. If it is
 ...
 * to be a subdirectory of picman_data_directory().
 *
 * The usual case is that no PICMAN2_DIRECTORY environment variable
 * exists, and then we use the PICMANDIR subdirectory of the local
 ...
 *
 * The returned string is owned by PICMAN and must not be modified or
 * freed. The returned string is in the encoding used for filenames by
 * GLib, which isn't necessarily UTF-8 (on Windows it is always
 * UTF-8.)
 *
 * Returns: The user-specific PICMAN settings directory.
 **/
const gchar *
picman_directory (void)
{
  static gchar *picman_dir          = NULL;
  static gchar *last_env_picman_dir = NULL;

  const gchar  *env_picman_dir;

  env_picman_dir = g_getenv ("PICMAN2_DIRECTORY");

  if (picman_dir)
    {
      gboolean picman2_directory_changed = FALSE;

      /* We have constructed the picman_dir already. We can return
       * picman_dir unless some parameter picman_dir depends on has
       * changed. For now we just check for changes to PICMAN2_DIRECTORY
       */
      picman2_directory_changed =
        (env_picman_dir == NULL &&
         last_env_picman_dir != NULL) ||
        (env_picman_dir != NULL &&
         last_env_picman_dir == NULL) ||
        (env_picman_dir != NULL &&
         last_env_picman_dir != NULL &&
         strcmp (env_picman_dir, last_env_picman_dir) != 0);

      if (! picman2_directory_changed)
        {
          return picman_dir;
        }
      else
        {
          /* Free the old picman_dir and go on to update it */
          g_free (picman_dir);
          picman_dir = NULL;
        }
    }

  /* Remember the PICMAN2_DIRECTORY to next invocation so we can check
   * if it changes
   */
  g_free (last_env_picman_dir);
  last_env_picman_dir = g_strdup (env_picman_dir);

  if (env_picman_dir)
    {
      if (g_path_is_absolute (env_picman_dir))
        {
          picman_dir = g_strdup (env_picman_dir);
        }
      else
        {
          ...
          if (home_dir)
            picman_dir = g_build_filename (home_dir, env_picman_dir, NULL);
          else
            picman_dir = g_build_filename (picman_data_directory (), env_picman_dir, NULL);
        }
    }
  else
    {
      ...
      picman_dir = g_build_filename ([library_dir UTF8String],
                                   PICMANDIR, PICMAN_USER_VERSION, NULL);
      ...
      picman_dir = g_build_filename (conf_dir,
                                   PICMANDIR, PICMAN_USER_VERSION, NULL);
      g_free(conf_dir);
       ...
       */
      picman_dir = g_build_filename (g_get_user_config_dir (),
                                   PICMANDIR, PICMAN_USER_VERSION, NULL);
...
static HMODULE libpicmanbase_dll = NULL;
...
/**
 * picman_installation_directory:
 *
 * Returns the top installation directory of PICMAN. On Unix the
 ...
 * resource path to check if PICMAN is part of a relocatable bundle.
 *
 * The returned string is owned by PICMAN and must not be modified or
 * freed. The returned string is in the encoding used for filenames by
 ...
 * Since: PICMAN 2.8
 *
 * Returns: The toplevel installation directory of PICMAN.
 **/
const gchar *
picman_installation_directory (void)
{
  ...
  toplevel = g_win32_get_package_installation_directory_of_module (libpicmanbase_dll);
  if (! toplevel)
    ...
    if (! strcmp (basename, ".libs"))
      {
        ...
        toplevel = _picman_reloc_find_prefix (PREFIX);
      }
      ...
      {
        /*  same for plug-ins, go three levels up from prefix/lib/picman/x.y  */
...
/**
 * picman_data_directory:
 *
 * Returns the top directory for PICMAN data. If the environment
 * variable PICMAN2_DATADIR exists, that is used.  It should be an
 ...
 * UTF-8.)
 *
 * Returns: The top directory for PICMAN data.
 **/
const gchar *
picman_data_directory (void)
{
  static gchar *picman_data_dir = NULL;
  ...
  if (! picman_data_dir)
    {
      ...
      picman_data_dir = picman_env_get_dir ("PICMAN2_DATADIR", DATADIR, tmp);
      g_free (tmp);
    }

  return picman_data_dir;
}

/**
 * picman_locale_directory:
 *
 * Returns the top directory for PICMAN locale files. If the environment
 * variable PICMAN2_LOCALEDIR exists, that is used.  It should be an
 * absolute pathname.  Otherwise, on Unix the compile-time defined
 * directory is used. On Windows, the installation directory as deduced
 * from the executable's full filename is used.
 *
 ...
 *
 * Returns: The top directory for PICMAN locale files.
 */
const gchar *
picman_locale_directory (void)
{
  static gchar *picman_locale_dir = NULL;

  if (! picman_locale_dir)
    {
      gchar *tmp = g_build_filename ("share",
                                     "locale",
                                     NULL);

      picman_locale_dir = picman_env_get_dir ("PICMAN2_LOCALEDIR", LOCALEDIR, tmp);
      g_free (tmp);

#ifdef G_OS_WIN32
      tmp = g_win32_locale_filename_from_utf8 (picman_locale_dir);
      g_free (picman_locale_dir);
      picman_locale_dir = tmp;
#endif
    }

  return picman_locale_dir;
}

/**
 * picman_sysconf_directory:
 *
 * Returns the top directory for PICMAN config files. If the environment
 * variable PICMAN2_SYSCONFDIR exists, that is used.  It should be an
 * absolute pathname.  Otherwise, on Unix the compile-time defined
 * directory is used. On Windows, the installation directory as deduced
 * from the executable's full filename is used.
 *
 ...
 * UTF-8.).
 *
 * Returns: The top directory for PICMAN config files.
 **/
const gchar *
picman_sysconf_directory (void)
{
  static gchar *picman_sysconf_dir = NULL;

  if (! picman_sysconf_dir)
    {
      gchar *tmp = g_build_filename ("etc",
                                     PICMAN_PACKAGE,
                                     PICMAN_SYSCONF_VERSION,
                                     NULL);

      picman_sysconf_dir = picman_env_get_dir ("PICMAN2_SYSCONFDIR", SYSCONFDIR, tmp);
      g_free (tmp);
    }

  return picman_sysconf_dir;
}

/**
 * picman_user_directory:
 ...
 **/
const gchar *
picman_user_directory (PicmanUserDirectory type)
{
  return g_get_user_special_dir (type);
}

/**
 * picman_plug_in_directory:
 *
 * Returns the top directory for PICMAN plug_ins and modules. If the
 * environment variable PICMAN2_PLUGINDIR exists, that is used.  It
 ...
 * UTF-8.)
 *
 * Returns: The top directory for PICMAN plug_ins and modules.
 **/
const gchar *
picman_plug_in_directory (void)
{
  static gchar *picman_plug_in_dir = NULL;

  if (! picman_plug_in_dir)
    {
      ...
      picman_plug_in_dir = picman_env_get_dir ("PICMAN2_PLUGINDIR", PLUGINDIR, tmp);
      g_free (tmp);
    }

  return picman_plug_in_dir;
}

/**
 * picman_personal_rc_file:
 * @basename: The basename of a rc_file.
 *
 * Returns the name of a file in the user-specific PICMAN settings directory.
 *
 ...
 **/
gchar *
picman_personal_rc_file (const gchar *basename)
{
  return g_build_filename (picman_directory (), basename, NULL);
}

/**
 * picman_gtkrc:
 *
 * Returns the name of PICMAN's application-specific gtkrc file.
 *
 * The returned string is owned by PICMAN and must not be modified or
 * freed. The returned string is in the encoding used for filenames by
 * GLib, which isn't necessarily UTF-8. (On Windows it always is
...
picman_gtkrc (void)
{
  static gchar *picman_gtkrc_filename = NULL;

  if (! picman_gtkrc_filename)
    picman_gtkrc_filename = g_build_filename (picman_data_directory (),
                                            "themes", "Default", "gtkrc",
                                            NULL);

  return picman_gtkrc_filename;
}

/**
 * picman_path_runtime_fix:
 ...
 */
static void
picman_path_runtime_fix (gchar **path)
{
  ...
  if (strncmp (*path, PREFIX "/", strlen (PREFIX "/")) == 0)
    {
       ...
       */
      p = *path;
      *path = g_strconcat (picman_installation_directory (),
                           "\\",
                           *path + strlen (PREFIX "/"),
                           NULL);
      g_free (p);
  ...
  if (!g_path_is_absolute (p))
    {
      *path = g_build_filename (picman_installation_directory (), *path, NULL);
      g_free (p);
    }
#else
  gchar *p;
  ...
  if (strncmp (*path, PREFIX G_DIR_SEPARATOR_S,
               strlen (PREFIX G_DIR_SEPARATOR_S)) == 0)
    {
       ...
       */
      p = *path;
      *path = g_build_filename (picman_installation_directory (),
 ...
 **/
GList *
picman_path_parse (const gchar  *path,
                 gint          max_paths,
                 gboolean      check,
                 GList       **check_failed)
{
...
#endif
        {
          picman_path_runtime_fix (&patharray[i]);
...
/**
 * picman_path_to_str:
 * @path: A list of directories as returned by picman_path_parse().
 *
 * Returns: A searchpath string separated by #G_SEARCHPATH_SEPARATOR.
 **/
gchar *
picman_path_to_str (GList *path)
{
  GString *str    = NULL;
  GList   *list;
  gchar   *retval = NULL;
 ...
 **/
gchar *
picman_path_get_user_writable_dir (GList *path)
{
  GList       *list;
  uid_t        euid;
  gid_t        egid;
...
static gchar *
picman_env_get_dir (const gchar *picman_env_name,
                  const gchar *compile_time_dir,
                  const gchar *relative_subdir)
{
  const gchar *env = g_getenv (picman_env_name);
  ...
  if (env)
  ...
  else if (compile_time_dir)
    {
      ...
      picman_path_runtime_fix (&retval);

      return retval;
    }

  return g_build_filename (picman_installation_directory (),
                           relative_subdir,
                           NULL);
}
