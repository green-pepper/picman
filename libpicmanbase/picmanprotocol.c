...
static void _gp_quit_write               (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_quit_destroy             (PicmanWireMessage  *msg);
...
static void _gp_config_write             (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_config_destroy           (PicmanWireMessage  *msg);
...
static void _gp_tile_req_write           (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_tile_req_destroy         (PicmanWireMessage  *msg);
...
static void _gp_tile_ack_write           (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_tile_ack_destroy         (PicmanWireMessage  *msg);
...
static void _gp_tile_data_write          (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_tile_data_destroy        (PicmanWireMessage  *msg);
...
static void _gp_proc_run_write           (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_proc_run_destroy         (PicmanWireMessage  *msg);
...
static void _gp_proc_return_write        (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_proc_return_destroy      (PicmanWireMessage  *msg);
...
static void _gp_temp_proc_run_write      (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_temp_proc_run_destroy    (PicmanWireMessage  *msg);
...
static void _gp_temp_proc_return_write   (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_temp_proc_return_destroy (PicmanWireMessage  *msg);
...
static void _gp_proc_install_write       (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_proc_install_destroy     (PicmanWireMessage  *msg);
...
static void _gp_proc_uninstall_write     (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_proc_uninstall_destroy   (PicmanWireMessage  *msg);
...
static void _gp_extension_ack_write      (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_extension_ack_destroy    (PicmanWireMessage  *msg);
...
static void _gp_has_init_write           (GIOChannel       *channel,
                                          PicmanWireMessage  *msg,
                                          gpointer          user_data);
static void _gp_has_init_destroy         (PicmanWireMessage  *msg);
               ...
               gpointer    user_data)
{
  ...
  if (! picman_wire_write_msg (channel, &msg, user_data))
    return FALSE;

  if (! picman_wire_flush (channel, user_data))
    return FALSE;
...
static void
_gp_quit_destroy (PicmanWireMessage *msg)
{
}
                 ...
                 gpointer         user_data)
{
  ...
  if (! _picman_wire_read_int32 (channel,
  ...
  if (! _picman_wire_read_int8 (channel,
    ...
    goto cleanup;
  if (! _picman_wire_read_int8 (channel,
                              (guint8 *) &config->picman_reserved_5, 1,
                              user_data))
    goto cleanup;
  if (! _picman_wire_read_int8 (channel,
                              (guint8 *) &config->picman_reserved_6, 1,
                              user_data))
    goto cleanup;
  if (! _picman_wire_read_int8 (channel,
                              (guint8 *) &config->picman_reserved_7, 1,
                              user_data))
    goto cleanup;
  if (! _picman_wire_read_int8 (channel,
                              (guint8 *) &config->picman_reserved_8, 1,
                              user_data))
    goto cleanup;
  if (! _picman_wire_read_int8 (channel,
  ...
  if (! _picman_wire_read_string (channel,
                  ...
                  gpointer         user_data)
{
  ...
  if (! _picman_wire_write_int32 (channel,
                                &config->version, 1, user_data))
    return;
  if (! _picman_wire_write_int32 (channel,
                                &config->tile_width, 1, user_data))
  ...
  if (! _picman_wire_write_int8 (channel,
    ...
    return;
  if (! _picman_wire_write_int8 (channel,
                               (const guint8 *) &config->picman_reserved_5, 1,
                               user_data))
    return;
  if (! _picman_wire_write_int8 (channel,
                               (const guint8 *) &config->picman_reserved_6, 1,
                               user_data))
    return;
  if (! _picman_wire_write_int8 (channel,
                               (const guint8 *) &config->picman_reserved_7, 1,
                               user_data))
    return;
  if (! _picman_wire_write_int8 (channel,
                               (const guint8 *) &config->picman_reserved_8, 1,
                               user_data))
    return;
  if (! _picman_wire_write_int8 (channel,
  ...
  if (! _picman_wire_write_int32 (channel,
                                ...
                                user_data))
    return;
  if (! _picman_wire_write_string (channel,
                                 &config->app_name, 1, user_data))
    return;
  if (! _picman_wire_write_string (channel,
                                 &config->wm_class, 1, user_data))
...
static void
_gp_config_destroy (PicmanWireMessage *msg)
{
  GPConfig *config = msg->data;
...
static void
_gp_tile_req_destroy (PicmanWireMessage *msg)
{
  GPTileReq *tile_req = msg->data;
...
static void
_gp_tile_ack_destroy (PicmanWireMessage *msg)
{
}
...
static void
_gp_tile_data_destroy (PicmanWireMessage *msg)
{
  GPTileData *tile_data = msg->data;
                   ...
                   gpointer         user_data)
{
  ...
  if (! _picman_wire_read_string (channel, &proc_run->name, 1, user_data))
    goto cleanup;
                    ...
                    gpointer         user_data)
{
  ...
  if (! _picman_wire_write_string (channel, &proc_run->name, 1, user_data))
    return;
...
static void
_gp_proc_run_destroy (PicmanWireMessage *msg)
{
  GPProcRun *proc_run = msg->data;
                      ...
                      gpointer         user_data)
{
  ...
  if (! _picman_wire_read_string (channel, &proc_return->name, 1, user_data))
    goto cleanup;
                       ...
                       gpointer         user_data)
{
  ...
  if (! _picman_wire_write_string (channel, &proc_return->name, 1, user_data))
    return;
...
static void
_gp_proc_return_destroy (PicmanWireMessage *msg)
{
  GPProcReturn *proc_return = msg->data;
...
static void
_gp_temp_proc_run_destroy (PicmanWireMessage *msg)
{
...
static void
_gp_temp_proc_return_destroy (PicmanWireMessage *msg)
{
...
static void
_gp_proc_install_destroy (PicmanWireMessage *msg)
{
  GPProcInstall *proc_install = msg->data;
                         ...
                         gpointer         user_data)
{
  ...
  if (! _picman_wire_read_string (channel, &proc_uninstall->name, 1, user_data))
    goto cleanup;
                          ...
                          gpointer         user_data)
{
  ...
  if (! _picman_wire_write_string (channel, &proc_uninstall->name, 1, user_data))
    return;
}

static void
_gp_proc_uninstall_destroy (PicmanWireMessage *msg)
{
  GPProcUninstall *proc_uninstall = msg->data;
...
static void
_gp_extension_ack_destroy (PicmanWireMessage *msg)
{
}
                 ...
                 gpointer     user_data)
{
  ...
  if (! _picman_wire_read_int32 (channel, (guint32 *) nparams, 1, user_data))
    return;
  ...
  for (i = 0; i < *nparams; i++)
    {
      ...
      switch ((*params)[i].type)
        ...
        case PICMAN_PDB_INT16:
          if (! _picman_wire_read_int16 (channel,
        ...
        case PICMAN_PDB_FLOAT:
          if (! _picman_wire_read_double (channel,
        ...
        case PICMAN_PDB_COLOR:
          if (! _picman_wire_read_color (channel,
	...
	case PICMAN_PDB_COLORARRAY:
	  (*params)[i].data.d_colorarray = g_new (PicmanRGB,
                                                  (*params)[i-1].data.d_int32);
	  if (! _picman_wire_read_color (channel,
  ...
  for (i = 0; i < nparams; i++)
    {
      ...
      switch (params[i].type)
        ...
        case PICMAN_PDB_INT16:
          if (! _picman_wire_write_int16 (channel,
        ...
        case PICMAN_PDB_FLOAT:
          if (! _picman_wire_write_double (channel,
                                         &params[i].data.d_float, 1,
                                         user_data))
            return;
          break;
        ...
        case PICMAN_PDB_COLOR:
          if (! _picman_wire_write_color (channel,
        ...
        case PICMAN_PDB_PARASITE:
          {
            PicmanParasite *p = &params[i].data.d_parasite;

            if (p->name == NULL)
              {
                /* write a null string to signal a null parasite */
                _picman_wire_write_string (channel,  &p->name, 1, user_data);
                break;
              }

            if (! _picman_wire_write_string (channel, &p->name, 1, user_data))
              return;
            if (! _picman_wire_write_int32 (channel, &p->flags, 1, user_data))
              return;
            if (! _picman_wire_write_int32 (channel, &p->size, 1, user_data))
              return;
            if (p->size > 0)
              {
                if (! _picman_wire_write_int8 (channel,
  ...
  for (i = 0; i < nparams; i++)
    {
      switch (params[i].type)
        ...
        case PICMAN_PDB_STRINGARRAY:
          if ((i > 0) && (params[i-1].type == PICMAN_PDB_INT32))
            {
...
static void
_gp_has_init_destroy (PicmanWireMessage *msg)
{
}
