...
/**
 * SECTION: picmanunitmenu
 * @title: PicmanUnitMenu
 * @short_description: Widget for selecting a #PicmanUnit.
 * @see_also: #PicmanUnit, #PicmanSizeEntry, picman_coordinates_new()
 *
 ...
 * passing a printf-like @format string to picman_unit_menu_new().
 *
 * The constructor also lets you choose if the menu should contain
 * items for PICMAN_UNIT_PIXEL, PICMAN_UNIT_PERCENT and a "More..." item
...
static void   picman_unit_menu_finalize (GObject   *object);
static void   picman_unit_menu_callback (GtkWidget *widget,
                                       gpointer   data);


G_DEFINE_TYPE (PicmanUnitMenu, picman_unit_menu, GTK_TYPE_OPTION_MENU)

#define parent_class picman_unit_menu_parent_class

static guint picman_unit_menu_signals[LAST_SIGNAL] = { 0 };


static void
picman_unit_menu_class_init (PicmanUnitMenuClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  /**
   * PicmanUnitMenu::unit-changed:
   *
   * This signal is emitted whenever the user selects a #PicmanUnit from
   * the #PicmanUnitMenu.
   **/
  picman_unit_menu_signals[UNIT_CHANGED] =
    g_signal_new ("unit-changed",
                  G_TYPE_FROM_CLASS (klass),
                  G_SIGNAL_RUN_FIRST,
                  G_STRUCT_OFFSET (PicmanUnitMenuClass, unit_changed),
                  NULL, NULL,
                  g_cclosure_marshal_VOID__VOID,
                  G_TYPE_NONE, 0);

  object_class->finalize = picman_unit_menu_finalize;

  klass->unit_changed    = NULL;
}

static void
picman_unit_menu_init (PicmanUnitMenu *menu)
{
  menu->format       = NULL;
  menu->unit         = PICMAN_UNIT_PIXEL;
...
static void
picman_unit_menu_finalize (GObject *object)
{
  PicmanUnitMenu *menu = PICMAN_UNIT_MENU (object);
 ...
 *
 * Creates a new #PicmanUnitMenu widget.
 *
 * For the @format string's possible expansions, see picman_unit_format_string().
 *
 * Returns: A pointer to the new #PicmanUnitMenu widget.
 **/
GtkWidget *
picman_unit_menu_new (const gchar *format,
                    PicmanUnit     unit,
                    gboolean     show_pixels,
                    gboolean     show_percent,
                    gboolean     show_custom)
{
  PicmanUnitMenu *unit_menu;
  GtkWidget    *menu;
  GtkWidget    *menuitem;
  gchar        *string;
  PicmanUnit      u;

  g_return_val_if_fail (((unit >= PICMAN_UNIT_PIXEL) &&
                         (unit < picman_unit_get_number_of_units ())) ||
                        (unit == PICMAN_UNIT_PERCENT), NULL);

  if ((unit >= picman_unit_get_number_of_built_in_units ()) &&
      (unit != PICMAN_UNIT_PERCENT))
    show_custom = TRUE;

  unit_menu = g_object_new (PICMAN_TYPE_UNIT_MENU, NULL);
  ...
  menu = gtk_menu_new ();
  for (u = show_pixels ? PICMAN_UNIT_PIXEL : PICMAN_UNIT_INCH;
       u < picman_unit_get_number_of_built_in_units ();
       u++)
    {
      /*  special cases "pixels" and "percent"  */
      if (u == PICMAN_UNIT_INCH)
        {
          if (show_percent)
            {
              string = picman_unit_format_string (format, PICMAN_UNIT_PERCENT);
              menuitem = gtk_menu_item_new_with_label (string);
              g_free (string);

              gtk_menu_shell_append (GTK_MENU_SHELL (menu), menuitem);
              g_object_set_data (G_OBJECT (menuitem), "picman_unit_menu",
                                 GINT_TO_POINTER (PICMAN_UNIT_PERCENT));
              gtk_widget_show (menuitem);

              g_signal_connect (menuitem, "activate",
                                G_CALLBACK (picman_unit_menu_callback),
                                unit_menu);
            }
      ...
      string = picman_unit_format_string (format, u);
      menuitem = gtk_menu_item_new_with_label (string);
      g_free (string);
  ...
  if ((unit >= picman_unit_get_number_of_built_in_units ()) &&
      (unit != PICMAN_UNIT_PERCENT))
    {
      ...
      string = picman_unit_format_string (format, unit);
      menuitem = gtk_menu_item_new_with_label (string);
      g_free (string);
  ...
  if (show_custom)
    {
      ...
      gtk_menu_shell_append (GTK_MENU_SHELL (menu), menuitem);
      g_object_set_data (G_OBJECT (menuitem), "picman_unit_menu",
                         GINT_TO_POINTER (PICMAN_UNIT_PERCENT + 1));
      gtk_widget_show (menuitem);
  ...
  gtk_option_menu_set_history (GTK_OPTION_MENU (unit_menu),
                               (unit == PICMAN_UNIT_PIXEL) ? 0 :
                               ((unit == PICMAN_UNIT_PERCENT) ?
                                (show_pixels ? 1 : 0) :
                                (((show_pixels || show_percent) ? 2 : 0) +
                                 ((show_pixels && show_percent) ? 1 : 0) +
                                 ((unit < PICMAN_UNIT_END) ?
                                  (unit - 1) : PICMAN_UNIT_END))));

  return GTK_WIDGET (unit_menu);
}

/**
 * picman_unit_menu_set_unit:
 * @menu:  The unit menu you want to set the unit for.
 * @unit: The new unit.
 *
 * Sets a new #PicmanUnit for the specified #PicmanUnitMenu.
 **/
void
picman_unit_menu_set_unit (PicmanUnitMenu *menu,
                         PicmanUnit      unit)
{
  ...
  g_return_if_fail (PICMAN_IS_UNIT_MENU (menu));
  g_return_if_fail (((unit >= PICMAN_UNIT_PIXEL) &&
                     ((unit > PICMAN_UNIT_PIXEL) || menu->show_pixels) &&
                     (unit < picman_unit_get_number_of_units ())) ||
                    ((unit == PICMAN_UNIT_PERCENT) && menu->show_percent));

  if (unit == menu->unit)
    return;

  items = GTK_MENU_SHELL (GTK_OPTION_MENU (menu)->menu)->children;
  user_unit = (PICMAN_UNIT_END +
               (((menu->show_pixels || menu->show_percent) ? 2 : 0) +
                ((menu->show_pixels && menu->show_percent) ? 1 : 0)));

  if ((unit >= PICMAN_UNIT_END) && (unit != PICMAN_UNIT_PERCENT))
    {
      gchar *string;
      ...
      string = picman_unit_format_string (menu->format, unit);
      menuitem = gtk_menu_item_new_with_label (string);
      g_free (string);
  ...
  g_signal_emit (menu, picman_unit_menu_signals[UNIT_CHANGED], 0);
}

/**
 * picman_unit_menu_get_unit:
 * @menu: The unit menu you want to know the unit of.
 *
 * Returns the #PicmanUnit the user has selected from the #PicmanUnitMenu.
 *
 * Returns: The unit the user has selected.
 **/
PicmanUnit
picman_unit_menu_get_unit (PicmanUnitMenu *menu)
{
  g_return_val_if_fail (PICMAN_IS_UNIT_MENU (menu), PICMAN_UNIT_INCH);
...
/**
 * picman_unit_menu_set_pixel_digits:
 * @menu: a #PicmanUnitMenu
 * @digits: the number of digits to display for a pixel size
 *
 * A PicmanUnitMenu can be setup to control the number of digits shown
 * by attached spinbuttons. Please refer to the documentation of
 * picman_unit_menu_update() to see how this is done.
 *
 ...
 **/
void
picman_unit_menu_set_pixel_digits (PicmanUnitMenu *menu,
                                 gint          digits)
{
  PicmanUnit unit;
  ...
  picman_unit_menu_update (GTK_WIDGET (menu), &unit);
}

/**
 * picman_unit_menu_get_pixel_digits:
 * @menu: a #PicmanUnitMenu
 *
 ...
 **/
gint
picman_unit_menu_get_pixel_digits (PicmanUnitMenu *menu)
{
  g_return_val_if_fail (PICMAN_IS_UNIT_MENU (menu), 0);

  return menu->pixel_digits;
}

/*  private callback of picman_unit_menu_create_selection ()  */
static void
picman_unit_menu_selection_response (GtkWidget    *widget,
                                   gint          response_id,
                                   PicmanUnitMenu *menu)
{
  if (response_id == GTK_RESPONSE_OK)
    {
      ...
      if (menu->selection && gtk_tree_selection_get_selected (sel, &model,
                                                              &iter))
        {
          ...
          unit = (PicmanUnit) g_value_get_int (&val);
          g_value_unset (&val);

          picman_unit_menu_set_unit (menu, unit);
        }
    }
...
static void
picman_unit_menu_selection_row_activated_callback (GtkTreeView       *tv,
                                                 GtkTreePath       *path,
...
/*  private function of picman_unit_menu_callback ()  */
static void
picman_unit_menu_create_selection (PicmanUnitMenu *menu)
{
  ...
  menu->selection = picman_dialog_new (_("Unit Selection"), "picman-unit-selection",
                                     parent, flags,
  ...
  g_signal_connect (menu->selection, "response",
                    G_CALLBACK (picman_unit_menu_selection_response),
                    menu);
  ...
  for (unit = PICMAN_UNIT_END; unit < num_units; unit++)
    {
      gchar *string;
      ...
      string = picman_unit_format_string ("(%f)", unit);
      gtk_list_store_set (list, &iter,
  ...
  g_signal_connect (menu->tv, "row-activated",
                    G_CALLBACK (picman_unit_menu_selection_row_activated_callback),
                    menu);
  ...
  if (menu->unit >= PICMAN_UNIT_END)
    {
      path = gtk_tree_path_new ();
      gtk_tree_path_append_index (path, menu->unit - PICMAN_UNIT_END);
...
static void
picman_unit_menu_callback (GtkWidget *widget,
                         gpointer   data)
{
  PicmanUnitMenu *menu = data;
  PicmanUnit      new_unit;
  ...
  /*  was "More..." selected?  */
  if (new_unit == (PICMAN_UNIT_PERCENT + 1))
    {
      gtk_option_menu_set_history (GTK_OPTION_MENU (menu),
                                   (menu->unit == PICMAN_UNIT_PIXEL) ? 0 :
                                   ((menu->unit == PICMAN_UNIT_PERCENT) ?
                                    (menu->show_pixels ? 1 : 0) :
                                    ((menu->show_pixels ||
                                      menu->show_percent ? 2 : 0) +
                                     (menu->show_pixels &&
                                      menu->show_percent ? 1 : 0) +
                                     ((menu->unit < PICMAN_UNIT_END) ?
                                      menu->unit - 1 : PICMAN_UNIT_END))));
      if (! menu->selection)
        picman_unit_menu_create_selection (menu);
      return;
    }
  ...
  picman_unit_menu_set_unit (menu, new_unit);
}
