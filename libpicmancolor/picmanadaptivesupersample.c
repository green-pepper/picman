...
#include "picmanadaptivesupersample.h"
#include "picmanrgb.h"


/**
 * SECTION: picmanadaptivesupersample
 * @title: PicmanAdaptiveSupersample
...
typedef struct _PicmanSampleType PicmanSampleType;

struct _PicmanSampleType
{
  guchar  ready;
  PicmanRGB color;
};
...
static gulong
picman_render_sub_pixel (gint             max_depth,
                       gint             depth,
                       PicmanSampleType **block,
                       gint             x,
                       gint             y,
                       gint             x1,
                       gint             y1,
  ...
  if (depth <= max_depth)
    {
      ...
      if ((picman_rgba_distance (&c[0], &c[1]) >= threshold) ||
          (picman_rgba_distance (&c[0], &c[2]) >= threshold) ||
          (picman_rgba_distance (&c[0], &c[3]) >= threshold) ||
          (picman_rgba_distance (&c[1], &c[2]) >= threshold) ||
          (picman_rgba_distance (&c[1], &c[3]) >= threshold) ||
          (picman_rgba_distance (&c[2], &c[3]) >= threshold))
        {
          ...
          num_samples += picman_render_sub_pixel (max_depth, depth + 1, block,
                                                x, y, x1, y1, x2, y2,
  ...
  if (c[0].a == 0.0 || c[1].a == 0.0 || c[2].a == 0.0 || c[3].a == 0.0)
    {
      ...
      picman_rgb_set (&tmpcol, 0.0, 0.0, 0.0);
...
gulong
picman_adaptive_supersample_area (gint              x1,
                                gint              y1,
                                gint              x2,
                                gint              y2,
                                gint              max_depth,
                                ...
                                gpointer          progress_data)
{
  ...
  PicmanRGB          color;                       /* Rendered pixel's color */
  PicmanSampleType   tmp_sample;                  /* For swapping samples */
  PicmanSampleType  *top_row, *bot_row, *tmp_row; /* Sample rows */
  PicmanSampleType **block;                       /* Sample block matrix */
  gulong           num_samples;
  ...
  top_row = g_new (PicmanSampleType, sub_pixel_size * width + 1);
  bot_row = g_new (PicmanSampleType, sub_pixel_size * width + 1);

  for (x = 0; x < (sub_pixel_size * width + 1); x++)
    {
      top_row[x].ready = FALSE;

      picman_rgba_set (&top_row[x].color, 0.0, 0.0, 0.0, 0.0);

      bot_row[x].ready = FALSE;

      picman_rgba_set (&bot_row[x].color, 0.0, 0.0, 0.0, 0.0);
    }

  /* Allocate block matrix */

  block = g_new (PicmanSampleType *, sub_pixel_size + 1); /* Rows */

  for (y = 0; y < (sub_pixel_size + 1); y++)
    {
      block[y] = g_new (PicmanSampleType, sub_pixel_size + 1); /* Columns */

      for (x = 0; x < (sub_pixel_size + 1); x++)
        {
          block[y][x].ready = FALSE;

          picman_rgba_set (&block[y][x].color, 0.0, 0.0, 0.0, 0.0);
        }
    }
      ...
      for (x = x1; x <= x2; x++)
        {
          ...
          num_samples += picman_render_sub_pixel (max_depth, 1, block, x, y, 0, 0,
